  ...
  class CanPublishCategory < ActiveInteraction::Base
    ...
    def check_dish_name(dish)
      locales.each do |locale|
        ...
        reason(:dish_missing_name, "Dish has no name for locale #{locale.inspect}", locale:, dish_id: dish.id) if name.to_s.blank?
      end
    end
    ...
    def category_not_root!
      reason(:category_not_root, "Category '#{category.name}' is not a root category: has a parent category '#{category.parent.name}'", { parent_category_id: category.parent.id })
    end
    ...
    def dish_invalid!(dish)
      reason(:dish_invalid, "Dish '#{dish.name}' is invalid: #{dish.errors.full_messages.join(', ')}", { dish_id: dish.id })
    end
