...
class CreateMenuDishesInCategories < ActiveRecord::Migration[7.0]
  def change
    create_table :menu_dishes_in_categories, id: :bigint do |t|
      ...
      t.index %i[index menu_category_id], unique: true, where: 'menu_category_id IS NOT NULL', name: 'index_index_menu_category_id'
      t.index %i[menu_dish_id menu_category_id], unique: true, where: 'menu_category_id IS NOT NULL', name: 'index_menu_dish_id_menu_category_id'
    end
  end
end
