...
RSpec.describe UserMailer do
  ...
  context "when calling #reset_password prepended #with(...)" do
    def mail(params = default_params)
      described_class.with(params).reset_password.deliver_now
    end

    it { expect(mail.to).to eq([user.email]) }

    it do
      expect(mail.subject).to eq I18n.t("user_mailer.reset_password.subject")
    end
    ...
    context "when checking text part body" do
      ...
      it { expect(body).not_to include(CGI.escapeHTML(I18n.t("mail.if_button_does_not_work_copy_and_paste_above"))) }

      it { expect(body).not_to include(I18n.t("mail.if_button_does_not_work_copy_and_paste_above")) }

      it { expect(body).not_to include("<a ") }

      it { expect(body).not_to include("href=") }

      it { expect(body).to include(token) }
      it { expect(body).to include(user.fullname) }

      it "when fullname is nil, should include email instead." do
        user.update!(fullname: nil)
        expect(body).to include(user.email)
      end

      it "when fullname is blank, should include email instead." do
    ...
    context "when checking html part body" do
      ...
      it { expect(body).to include(CGI.escapeHTML(I18n.t("mail.if_button_does_not_work_copy_and_paste_above"))) }

      it { expect(body).to include("<a ") }

      it { expect(body).to include("href=") }
    ...
    %w[it en].each do |lang|
      context "when language is #{lang.inspect}" do
        ...
        it do
          I18n.with_locale(lang) do
            expect(mail.text_part.body.encoded).not_to include I18n.t("user_mailer.reset_password.subject")
          end
        end

        it do
          I18n.with_locale(lang) do
            expect(mail.html_part.body.encoded).to include CGI.escapeHTML(I18n.t("user_mailer.reset_password.body"))
          end
        end

        it do
          I18n.with_locale(lang) do
            expect(mail.text_part.body.encoded).to include I18n.t("user_mailer.reset_password.body")
          end
        end
      end
    end
