...
RSpec.describe ReservationMailer do
  ...
  let(:reservation) { create(:reservation, :with_fullname, datetime: 1.day.ago) }
  ...
  [
    ...
    context "when calling #feedback with params #{params.inspect}" do
      let(:mail) { described_class.with(params).feedback.deliver_now }
  ...
  [
    ...
    context "when email is missing or invalid (#{missing_mail.inspect})" do
      ...
      let(:mail) do
        described_class.with(default_params).feedback.deliver_now
      end
  ...
  context "when calling #feedback prepended #with(...)" do
    around do |example|
    ...
    it do
      expect(mail.subject).to eq I18n.t("reservation_mailer.feedback.subject", fullname: reservation.fullname)
    end
    ...
    it do
      ...
      expect(Log::DeliveredEmail.last.controller_path).to eq("reservation_mailer")
      expect(Log::DeliveredEmail.last.action_name).to eq("feedback")
    end
    ...
    context "when checking text part body" do
      ...
      it { expect(body).to include("http") }

      it do
        expect(body).to include(
          Rails.application.routes.url_helpers.feedback_reservations_url(secret: reservation.secret)
        )
      end

      it { expect(body).to include(reservation.secret) }
    end
      ...
      context "when language is #{lang}" do
        ...
        it do
          expect(mail.text_part.body.encoded).to include(
            I18n.t(
              "reservation_mailer.feedback.body",
