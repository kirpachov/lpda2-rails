...
RSpec.describe V1::ReservationsController, type: :controller do
  ...
  context 'POST #create' do
    ...
    let!(:turn) { create(:reservation_turn, starts_at: DateTime.parse('00:01'), ends_at: DateTime.parse('23:59'), weekday: 0) }
    ...
    let(:phone) { [Faker::PhoneNumber.cell_phone, Faker::PhoneNumber.cell_phone_in_e164, Faker::PhoneNumber.cell_phone_with_country_code].sample }
    ...
    context 'basic' do
      ...
      context 'when turn is not in that weekday' do
        let!(:turn) { create(:reservation_turn, starts_at: DateTime.parse('00:01'), ends_at: DateTime.parse('23:59'), weekday: 1) }
      ...
      context 'when already exists a reservation for same datetime and email' do
        let!(:reservation) { create(:reservation, datetime: datetime, email: email) }
  ...
  context 'PATCH #cancel' do
    ...
    it { should route(:patch, '/v1/reservations/cancel').to(format: :json, action: :cancel, controller: 'v1/reservations') }
  ...
  context 'GET #show' do
    ...
    it { should route(:get, '/v1/reservations/supersecret').to(format: :json, action: :show, controller: 'v1/reservations', secret: 'supersecret') }
