...
RSpec.describe Menu::Tag, type: :model do
  ...
  context 'associations' do
    ...
    it { should have_many(:menu_dishes).through(:menu_tags_in_dishes) }
    ...
    context 'when deleted, should not delete associated dishes, but should delete association to dish itself' do
      ...
      it { expect { menu_tag.destroy }.not_to change(Menu::Dish, :count) }
      it { expect { menu_tag.destroy }.to change(Menu::Tag, :count).by(-1) }
      it { expect { menu_tag.destroy }.to change(Menu::TagsInDish, :count).by(-1) }
    end
  end
end
