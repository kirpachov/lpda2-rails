...
545 files inspected, 4787 offenses detected:
...
### app/controllers/v1/admin/reservations_controller.rb - (12 offenses)
  * **Line # 4 - convention:** Metrics/ClassLength: Class has too many lines. [176/100]
  ...
  * **Line # 42 - convention:** Metrics/AbcSize: Assignment Branch Condition size for ungrouped_tables_summary is too high. [<1, 17, 1> 17.06/17]

    ```rb
        def ungrouped_tables_summary ...
    ```

  * **Line # 50 - convention:** Layout/TrailingWhitespace: Trailing whitespace detected.

    ```rb
          render json: Reservation.where.not(status: %i[deleted cancelled 
    ```

  * **Line # 50 - convention:** Layout/LineLength: Line is too long. [148/120]
  ...
  * **Line # 51 - convention:** Layout/ArrayAlignment: Align the elements of an array literal if they span more than one line.

    ```rb
    noshow]).where(id: search.result.select(:id)).group("children + adults").count
    ```

  * **Line # 51 - convention:** Layout/LineLength: Line is too long. [130/120]

    ```rb
                                                        noshow]).where(id: search.result.select(:id)).group("children + adults").count
    ```

  * **Line # 161 - convention:** Metrics/AbcSize: Assignment Branch Condition size for export is too high. [<2, 20, 2> 20.2/17]

    ```rb
        def export ...
    ```

  * **Line # 161 - convention:** Metrics/MethodLength: Method has too many lines. [12/10]

    ```rb
        def export ...
    ```

  * **Line # 191 - convention:** Layout/LineLength: Line is too long. [139/120]

    ```rb
                                                            table_type: [:text_translations, { images: [:attached_image_blob] }]).map do |item|
    ```

  * **Line # 202 - convention:** Metrics/MethodLength: Method has too many lines. [19/10]
...
### app/controllers/v1/reservations_controller.rb - (18 offenses)
  * **Line # 4 - convention:** Metrics/ClassLength: Class has too many lines. [109/100]
  ...
  * **Line # 87 - convention:** Metrics/AbcSize: Assignment Branch Condition size for datetime_requires_payment is too high. [<2, 41, 5> 41.35/17]

    ```rb
        def datetime_requires_payment ...
    ```

  * **Line # 87 - convention:** Metrics/MethodLength: Method has too many lines. [23/10]
  ...
  * **Line # 100 - convention:** Layout/LineLength: Line is too long. [129/120]

    ```rb
              table_type_to_preorder_reservation_groups: call.result.table_type_to_preorder_reservation_groups.includes(table_type: [
    ```

  * **Line # 101 - convention:** Layout/FirstArrayElementIndentation: Use 2 spaces for indentation in an array, relative to the first position after the preceding left parenthesis.

    ```rb
    :text_translations, { images: [:attached_image_blob] }]).map do |tt|
    ```

  * **Line # 101 - convention:** Layout/MultilineArrayBraceLayout: The closing array brace must be on the line after the last array element when the opening brace is on a separate line from the first array element.

    ```rb
    :text_translations, { images: [:attached_image_blob] }]).map do |tt|
    ```

  * **Line # 101 - convention:** Layout/LineLength: Line is too long. [172/120]

    ```rb
                                                                                                                          :text_translations, { images: [:attached_image_blob] }
    ```

  * **Line # 102 - convention:** Layout/FirstArrayElementIndentation: Indent the right bracket the same as the first position after the preceding left parenthesis.

    ```rb
    ]).map do |tt|
    ```

  * **Line # 102 - convention:** Layout/LineLength: Line is too long. [130/120]

    ```rb
                                                                                                                        ]).map do |tt|
    ```

  * **Line # 102 - convention:** Layout/LineLength: Line is too long. [197/120]
  ...
  * **Line # 103 - convention:** Layout/IndentationWidth: Use 2 (not -41) spaces for indentation.
  ...
  * **Line # 110 - convention:** Layout/BlockAlignment: `end` at 110, 10 is not aligned with `call.result.table_type_to_preorder_reservation_groups.includes(table_type: [` at 100, 53 or `:text_translations, { images: [:attached_image_blob] }]).map do |tt|` at 101, 0.
...
### app/interactions/reservation_turn_valid_times.rb - (3 offenses)
  * **Line # 11 - convention:** Metrics/AbcSize: Assignment Branch Condition size for execute is too high. [<16, 40, 10> 44.23/17]

    ```rb
      def execute ...
    ```

  * **Line # 11 - convention:** Metrics/CyclomaticComplexity: Cyclomatic complexity for execute is too high. [8/7]
...
### spec/controllers/v1/admin/reservations_controller/reservations_controller.tables_summary_spec.rb - (9 offenses)
  ...
  * **Line # 173 - convention:** Rails/Date: Do not use `Date.today` without zone. Use `Time.zone.today` instead.
...
### spec/controllers/v1/reservations_controller/reservations_controller.cancel_spec.rb - (27 offenses)
  ...
  * **Line # 198 - convention:** Layout/LineLength: Line is too long. [144/120]
  ...
  * **Line # 202 - convention:** Performance/CollectionLiteralInLoop: Avoid immutable Array literals in loops. It is better to extract it into a local variable or a constant.

    ```rb
                create(:reservation_payment, status: %w[authorized paid].sample, reservation: r)
    ```

  * **Line # 209 - convention:** Performance/CollectionLiteralInLoop: Avoid immutable Array literals in loops. It is better to extract it into a local variable or a constant.

    ```rb
            [ ...
    ```

  * **Line # 242 - convention:** Performance/CollectionLiteralInLoop: Avoid immutable Array literals in loops. It is better to extract it into a local variable or a constant.
  ...
  * **Line # 281 - convention:** Performance/CollectionLiteralInLoop: Avoid immutable Array literals in loops. It is better to extract it into a local variable or a constant.

    ```rb
                %w[paid todo refunded authorized].each do |pstatus|
    ```

  * **Line # 285 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].

    ```rb
                    it do
    ```

  * **Line # 301 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].

    ```rb
            it do
    ```

  * **Line # 333 - convention:** RSpec/AnyInstance: Avoid stubbing using `allow_any_instance_of`.

    ```rb
              allow_any_instance_of(Reservation).to receive(:cancelled!).and_return(false)
    ```

  * **Line # 336 - convention:** RSpec/AnyInstance: Avoid stubbing using `allow_any_instance_of`.

    ```rb
              allow_any_instance_of(Reservation).to receive(:errors).and_return(errors)
    ```

  * **Line # 342 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].
...
### spec/requests/v1/reservations_controller/datetime_requires_payment_spec.rb - (78 offenses)
  ...
  * **Line # 15 - convention:** RSpec/EmptyLineAfterExample: Add an empty line after `it`.

    ```rb
        it { expect(json[:preorder_reservation_group][:table_type_to_preorder_reservation_groups].length).to be_positive }
    ```

  * **Line # 16 - convention:** Layout/LineLength: Line is too long. [144/120]
  ...
  * **Line # 17 - convention:** RSpec/EmptyLineAfterExample: Add an empty line after `it`.
  ...
  * **Line # 17 - convention:** Layout/LineLength: Line is too long. [145/120]
  ...
  * **Line # 17 - convention:** Layout/BlockEndNewline: Expression at 17, 136 should be on its own line.

    ```rb
     expect(json[:preorder_reservation_group][:table_type_to_preorder_reservation_groups].pluck(:table_type)).to all(include(id: Integer)) }
    ```

  * **Line # 18 - convention:** Layout/IndentationWidth: Use 2 (not 1) spaces for indentation.

    ```rb
     expect(json[:preorder_reservation_group][:table_type_to_preorder_reservation_groups].pluck(:table_type)).to all(include(id: Integer))
    ```

  * **Line # 18 - convention:** Layout/IndentationWidth: Use 2 (not -2) spaces for indentation.

    ```rb
      expect(json[:preorder_reservation_group][:table_type_to_preorder_reservation_groups].pluck(:table_type)).to all(include(id: Integer))
    ```

  * **Line # 18 - convention:** Layout/LineLength: Line is too long. [139/120]

    ```rb
          expect(json[:preorder_reservation_group][:table_type_to_preorder_reservation_groups].pluck(:table_type)).to all(include(id: Integer))
    ```

  * **Line # 18 - convention:** Layout/LineLength: Line is too long. [152/120]
  ...
  * **Line # 19 - convention:** Layout/BlockAlignment: `}` at 19, 0 is not aligned with `it {` at 17, 4.

    ```rb
    }
    ```

  * **Line # 19 - convention:** RSpec/EmptyLineAfterExample: Add an empty line after `it`.
  ...
  * **Line # 19 - convention:** Layout/LineLength: Line is too long. [151/120]
  ...
  * **Line # 19 - convention:** Layout/BlockEndNewline: Expression at 19, 137 should be on its own line.

    ```rb
     expect(json[:preorder_reservation_group][:table_type_to_preorder_reservation_groups].pluck(:table_type)).to all(include(name: String)) }
    ```

  * **Line # 20 - convention:** Layout/LineLength: Line is too long. [146/120]
  ...
  * **Line # 21 - convention:** RSpec/EmptyLineAfterExample: Add an empty line after `it`.
  ...
  * **Line # 21 - convention:** Layout/LineLength: Line is too long. [149/120]
  ...
  * **Line # 21 - convention:** Layout/BlockEndNewline: Expression at 21, 144 should be on its own line.

    ```rb
     expect(json[:preorder_reservation_group][:table_type_to_preorder_reservation_groups].pluck(:table_type)).to all(include(description: String)) }
    ```

  * **Line # 22 - convention:** Layout/IndentationWidth: Use 2 (not 1) spaces for indentation.

    ```rb
     expect(json[:preorder_reservation_group][:table_type_to_preorder_reservation_groups].pluck(:table_type)).to all(include(name: String))
    ```

  * **Line # 22 - convention:** Layout/IndentationWidth: Use 2 (not -2) spaces for indentation.

    ```rb
      expect(json[:preorder_reservation_group][:table_type_to_preorder_reservation_groups].pluck(:table_type)).to all(include(name: String))
    ```

  * **Line # 22 - convention:** Layout/LineLength: Line is too long. [140/120]

    ```rb
          expect(json[:preorder_reservation_group][:table_type_to_preorder_reservation_groups].pluck(:table_type)).to all(include(name: String))
    ```

  * **Line # 22 - convention:** Layout/LineLength: Line is too long. [185/120]
  ...
  * **Line # 23 - convention:** Layout/BlockAlignment: `}` at 23, 0 is not aligned with `it {` at 21, 4.
  ...
  * **Line # 23 - convention:** Layout/BlockEndNewline: Expression at 23, 143 should be on its own line.

    ```rb
     expect(json[:preorder_reservation_group][:table_type_to_preorder_reservation_groups].pluck(:table_type)).to all(include(translations: Hash)) }
    ```

  * **Line # 25 - convention:** RSpec/EmptyLineAfterExample: Add an empty line after `it`.
  ...
  * **Line # 25 - convention:** Layout/BlockEndNewline: Expression at 25, 138 should be on its own line.

    ```rb
     expect(json[:preorder_reservation_group][:table_type_to_preorder_reservation_groups].pluck(:table_type)).to all(include(images: Array)) }
    ```

  * **Line # 26 - convention:** Layout/IndentationWidth: Use 2 (not 1) spaces for indentation.
  ...
  * **Line # 26 - convention:** Layout/LineLength: Line is too long. [147/120]

    ```rb
          expect(json[:preorder_reservation_group][:table_type_to_preorder_reservation_groups].pluck(:table_type)).to all(include(description: String))
    ```

  * **Line # 27 - convention:** Layout/BlockAlignment: `}` at 27, 0 is not aligned with `it {` at 25, 4.

    ```rb
    }
    ```

  * **Line # 27 - convention:** RSpec/EmptyLineAfterExample: Add an empty line after `it`.
  ...
  * **Line # 27 - convention:** Layout/BlockEndNewline: Expression at 27, 141 should be on its own line.

    ```rb
     expect(json[:preorder_reservation_group][:table_type_to_preorder_reservation_groups].pluck(:table_type).pluck(:images)).to all(be_present) }
    ```

  * **Line # 28 - convention:** RSpec/SharedContext: Use `shared_examples` when you don't define context.
  ...
  * **Line # 29 - convention:** Layout/LineLength: Line is too long. [177/120]
    ...
    ```rb
     expect(json[:preorder_reservation_group][:table_type_to_preorder_reservation_groups].pluck(:table_type).pluck(:images).flatten).to all(include(filename: String, url: String)) }
    ```

  * **Line # 29 - convention:** Layout/BlockEndNewline: Expression at 29, 177 should be on its own line.

    ```rb
     expect(json[:preorder_reservation_group][:table_type_to_preorder_reservation_groups].pluck(:table_type).pluck(:images).flatten).to all(include(filename: String, url: String)) }
    ```

  * **Line # 30 - convention:** Layout/IndentationWidth: Use 2 (not 1) spaces for indentation.

    ```rb
     expect(json[:preorder_reservation_group][:table_type_to_preorder_reservation_groups].pluck(:table_type)).to all(include(translations: Hash))
    ```

  * **Line # 30 - convention:** Layout/IndentationWidth: Use 2 (not -2) spaces for indentation.

    ```rb
      expect(json[:preorder_reservation_group][:table_type_to_preorder_reservation_groups].pluck(:table_type)).to all(include(translations: Hash))
    ```

  * **Line # 30 - convention:** Layout/LineLength: Line is too long. [146/120]

    ```rb
          expect(json[:preorder_reservation_group][:table_type_to_preorder_reservation_groups].pluck(:table_type)).to all(include(translations: Hash))
    ```

  * **Line # 31 - convention:** Layout/BlockAlignment: `}` at 31, 0 is not aligned with `it {` at 29, 4.
  ...
  * **Line # 34 - convention:** Layout/IndentationWidth: Use 2 (not -2) spaces for indentation.

    ```rb
      expect(json[:preorder_reservation_group][:table_type_to_preorder_reservation_groups].pluck(:table_type)).to all(include(images: Array))
    ```

  * **Line # 34 - convention:** Layout/LineLength: Line is too long. [141/120]

    ```rb
          expect(json[:preorder_reservation_group][:table_type_to_preorder_reservation_groups].pluck(:table_type)).to all(include(images: Array))
    ```

  * **Line # 35 - convention:** Layout/BlockAlignment: `}` at 35, 0 is not aligned with `it {` at 33, 4.
  ...
  * **Line # 38 - convention:** Layout/IndentationWidth: Use 2 (not -2) spaces for indentation.

    ```rb
      expect(json[:preorder_reservation_group][:table_type_to_preorder_reservation_groups].pluck(:table_type).pluck(:images)).to all(be_present)
    ```

  * **Line # 38 - convention:** Layout/LineLength: Line is too long. [144/120]

    ```rb
          expect(json[:preorder_reservation_group][:table_type_to_preorder_reservation_groups].pluck(:table_type).pluck(:images)).to all(be_present)
    ```

  * **Line # 39 - convention:** Layout/BlockAlignment: `}` at 39, 0 is not aligned with `it {` at 37, 4.

    ```rb
    }
    ```

  * **Line # 42 - convention:** Layout/IndentationWidth: Use 2 (not 1) spaces for indentation.
  ...
  * **Line # 42 - convention:** Layout/IndentationWidth: Use 2 (not -2) spaces for indentation.

    ```rb
      expect(json[:preorder_reservation_group][:table_type_to_preorder_reservation_groups].pluck(:table_type).pluck(:images).flatten).to all(include(
    ```

  * **Line # 42 - convention:** Layout/LineLength: Line is too long. [149/120]

    ```rb
          expect(json[:preorder_reservation_group][:table_type_to_preorder_reservation_groups].pluck(:table_type).pluck(:images).flatten).to all(include(
    ```

  * **Line # 43 - convention:** Layout/FirstArgumentIndentation: Indent the first argument one step more than `include(`.

    ```rb
    filename: String, url: String))
    ```

  * **Line # 43 - convention:** Layout/MultilineMethodCallBraceLayout: Closing method call brace must be on the line after the last argument when opening brace is on a separate line from the first argument.

    ```rb
    filename: String, url: String))
    ```

  * **Line # 43 - convention:** Layout/LineLength: Line is too long. [172/120]

    ```rb
                                                                                                                                                   filename: String, url: String
    ```

  * **Line # 44 - convention:** Layout/BlockAlignment: `}` at 44, 0 is not aligned with `it {` at 41, 4.

    ```rb
    }
    ```

  * **Line # 44 - convention:** Layout/ClosingParenthesisIndentation: Indent `)` to column 137 (not 0)

    ```rb
    ))
    ```

  * **Line # 44 - convention:** Layout/LineLength: Line is too long. [143/120]

    ```rb
                                                                                                                                                 ))
    ```

  * **Line # 56 - convention:** RSpec/Rails/InferredSpecType: Remove redundant spec type.
  ...
  * **Line # 69 - convention:** RSpec/ScatteredLet: Group all let/let! blocks in the example group together.

    ```rb
      let(:default_params) { { date:, time:, people: } }
    ```

  * **Line # 70 - convention:** RSpec/ScatteredLet: Group all let/let! blocks in the example group together.

    ```rb
      let(:default_headers) { {} }
    ```

  * **Line # 78 - convention:** Layout/EmptyLines: Extra blank line detected.

    ```rb
     ...
    ```

  * **Line # 84 - convention:** Style/HashSyntax: Omit the hash value.
  ...
  * **Line # 84 - convention:** Layout/LineLength: Line is too long. [136/120]
  ...
  * **Line # 93 - convention:** Style/HashSyntax: Omit the hash value.

    ```rb
          group.add_table_type(table_type: table_type, price: table_type.default_price, people_per_turn: table_type.default_people_per_turn)
    ```

  * **Line # 93 - convention:** Layout/LineLength: Line is too long. [136/120]

    ```rb
          group.add_table_type(table_type: table_type, price: table_type.default_price, people_per_turn: table_type.default_people_per_turn)
    ```

  * **Line # 101 - convention:** Layout/TrailingWhitespace: Trailing whitespace detected.
  ...
  * **Line # 106 - convention:** Layout/ArgumentAlignment: Align the arguments of a method call if they span more than one line.
  ...
  * **Line # 116 - convention:** Layout/ArgumentAlignment: Align the arguments of a method call if they span more than one line.
...
### spec/requests/v2/reservations_controller/valid_times_spec.rb - (73 offenses)
  ...
  * **Line # 929 - convention:** RSpec/RepeatedExample: Don't repeat examples within an example group.
