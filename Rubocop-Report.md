...
530 files inspected, 4649 offenses detected:
...
### app/controllers/v1/admin/reservations_controller.rb - (8 offenses)
  ...
  * **Line # 190 - convention:** Layout/ArgumentAlignment: Align the arguments of a method call if they span more than one line.
...
### app/interactions/create_preorder_group.rb - (7 offenses)
  ...
  * **Line # 25 - convention:** Metrics/AbcSize: Assignment Branch Condition size for execute is too high. [<0, 21, 6> 21.84/17]

    ```rb
      def execute ...
    ```

  * **Line # 41 - convention:** Metrics/AbcSize: Assignment Branch Condition size for create_group is too high. [<1, 23, 2> 23.11/17]

    ```rb
      def create_group ...
    ```

  * **Line # 41 - convention:** Metrics/MethodLength: Method has too many lines. [16/10]

    ```rb
      def create_group ...
    ```

  * **Line # 95 - convention:** Metrics/AbcSize: Assignment Branch Condition size for initialize_table_types is too high. [<3, 19, 4> 19.65/17]
  ...
  * **Line # 96 - convention:** Style/PreferredHashMethods: Use `Hash#key?` instead of `Hash#has_key?`.
...
### app/interactions/create_reservation_payment_by_group.rb - (1 offense)
  * **Line # 5 - convention:** Layout/EmptyLinesAroundClassBody: Extra empty line detected at class body beginning.
...
### app/interactions/nexi/record_deferred_payment.rb - (8 offenses)
  ...
  * **Line # 40 - convention:** Style/TrailingCommaInArguments: Avoid comma after the last parameter of a method call.

    ```rb
            mac_part: "apiKey=#{params[:apiKey]}codiceTransazione=#{params[:codiceTransazione]}divisa=#{params[:divisa]}importo=#{params[:importo]}timeStamp=#{params[:timeStamp]}",
    ```

  * **Line # 58 - convention:** Style/TrailingCommaInHashLiteral: Avoid comma after the last item of a hash.
...
### app/interactions/public_create_reservation.rb - (12 offenses)
  * **Line # 3 - convention:** Metrics/ClassLength: Class has too many lines. [239/100]
  ...
  * **Line # 38 - convention:** Metrics/AbcSize: Assignment Branch Condition size for execute is too high. [<1, 35, 9> 36.15/17]
  ...
  * **Line # 38 - convention:** Metrics/MethodLength: Method has too many lines. [16/10]
  ...
  * **Line # 130 - convention:** Metrics/MethodLength: Method has too many lines. [15/10]

    ```rb
      def initialize_reservation ...
    ```

  * **Line # 164 - convention:** Metrics/AbcSize: Assignment Branch Condition size for create_reservation_payment_if_needed is too high. [<4, 30, 8> 31.3/17]

    ```rb
      def create_reservation_payment_if_needed ...
    ```

  * **Line # 164 - convention:** Metrics/MethodLength: Method has too many lines. [23/10]

    ```rb
      def create_reservation_payment_if_needed ...
    ```

  * **Line # 180 - convention:** Style/NumericPredicate: Use `call.result.negative?` instead of `call.result < 0`.

    ```rb
          if call.valid? && call.result < 0
    ```

  * **Line # 280 - convention:** Metrics/AbcSize: Assignment Branch Condition size for datetime_format_is_valid is too high. [<0, 17, 3> 17.26/17]

    ```rb
      def datetime_format_is_valid ...
    ```

  * **Line # 332 - convention:** Style/NumericPredicate: Use `(datetime.to_i % reservation_turn.step).zero?` instead of `datetime.to_i % reservation_turn.step == 0`.
...
### app/interactions/update_preorder_group.rb - (10 offenses)
  ...
  * **Line # 28 - convention:** Metrics/AbcSize: Assignment Branch Condition size for execute is too high. [<0, 22, 6> 22.8/17]

    ```rb
      def execute ...
    ```

  * **Line # 43 - convention:** Metrics/AbcSize: Assignment Branch Condition size for update_group is too high. [<2, 24, 3> 24.27/17]
  ...
  * **Line # 63 - convention:** Style/PreferredHashMethods: Use `Hash#key?` instead of `Hash#has_key?`.

    ```rb
        return [] unless params.has_key?(:dates)
    ```

  * **Line # 73 - convention:** Metrics/AbcSize: Assignment Branch Condition size for update_turns is too high. [<4, 17, 5> 18.17/17]

    ```rb
      def update_turns ...
    ```

  * **Line # 74 - convention:** Style/PreferredHashMethods: Use `Hash#key?` instead of `Hash#has_key?`.

    ```rb
        return true unless params.has_key?(:turns)
    ```

  * **Line # 101 - convention:** Metrics/AbcSize: Assignment Branch Condition size for initialize_table_types is too high. [<3, 22, 4> 22.56/17]

    ```rb
      def initialize_table_types ...
    ```

  * **Line # 101 - convention:** Metrics/MethodLength: Method has too many lines. [14/10]

    ```rb
      def initialize_table_types ...
    ```

  * **Line # 102 - convention:** Style/PreferredHashMethods: Use `Hash#key?` instead of `Hash#has_key?`.
...
### app/mailers/reservation_mailer.rb - (7 offenses)
  ...
  * **Line # 37 - convention:** Layout/TrailingWhitespace: Trailing whitespace detected.

    ```rb
          subject: (@title = I18n.t("reservation_mailer.payment_required_to_confirm.subject", 
    ```

  * **Line # 37 - convention:** Layout/LineLength: Line is too long. [123/120]

    ```rb
          subject: (@title = I18n.t("reservation_mailer.payment_required_to_confirm.subject", fullname: reservation.fullname)),
    ```

  * **Line # 38 - convention:** Layout/ArgumentAlignment: Align the arguments of a method call if they span more than one line.
...
### app/models/reservation.rb - (7 offenses)
  * **Line # 3 - convention:** Metrics/ClassLength: Class has too many lines. [103/100]
  ...
  * **Line # 75 - convention:** Layout/LineLength: Line is too long. [166/120]
...
### app/models/reservation_payment.rb - (3 offenses)
  ...
  * **Line # 67 - convention:** Style/Alias: Use `alias` instead of `alias_method` in a class body.

    ```rb
      alias_method :deferred, :deferred?
    ```

  * **Line # 67 - convention:** Style/Alias: Use `alias deferred deferred?` instead of `alias :deferred :deferred?`.
...
### config/routes.rb - (6 offenses)
  * **Line # 6 - convention:** Metrics/BlockLength: Block has too many lines. [181/25]

    ```rb
    Rails.application.routes.draw do ...
    ```

  * **Line # 17 - convention:** Metrics/BlockLength: Block has too many lines. [169/25]

    ```rb
      defaults format: :json do ...
    ```

  * **Line # 26 - convention:** Metrics/BlockLength: Block has too many lines. [160/25]

    ```rb
        scope module: :v1, path: "v1" do ...
    ```

  * **Line # 95 - convention:** Metrics/BlockLength: Block has too many lines. [110/25]

    ```rb
          scope module: :admin, path: "admin" do ...
    ```

  * **Line # 156 - convention:** Metrics/BlockLength: Block has too many lines. [56/25]

    ```rb
            scope module: :menu, path: "menu" do ...
    ```

  * **Line # 192 - convention:** Metrics/BlockLength: Block has too many lines. [26/25]
...
### spec/controllers/v1/admin/reservations_controller_spec.rb - (123 offenses)
  * **Line # 7 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].
  ...
  * **Line # 99 - convention:** Layout/LineLength: Line is too long. [131/120]

    ```rb
                                                                 default_people_per_turn: Integer, default_price: Float, images: Array)
    ```

  * **Line # 117 - convention:** Performance/InefficientHashSearch: Use `#key?` instead of `#keys.include?`.

    ```rb
              it { expect(json[:items].filter { |j| j.keys.include?("payment") }.filter(&:present?).count).to eq(1) }
    ```

  * **Line # 117 - convention:** Performance/Count: Use `count` instead of `filter...count`.

    ```rb
              it { expect(json[:items].filter { |j| j.keys.include?("payment") }.filter(&:present?).count).to eq(1) }
    ```

  * **Line # 118 - convention:** Performance/Detect: Use `find` instead of `filter.first`.

    ```rb
              it { expect(json[:items].filter { |j| j.keys.include?("payment") }.first).to be_present }
    ```

  * **Line # 118 - convention:** Performance/InefficientHashSearch: Use `#key?` instead of `#keys.include?`.

    ```rb
              it { expect(json[:items].filter { |j| j.keys.include?("payment") }.first).to be_present }
    ```

  * **Line # 125 - convention:** Performance/Detect: Use `find` instead of `filter.first`.

    ```rb
                expect(json[:items].filter do |j| ...
    ```

  * **Line # 126 - convention:** Performance/InefficientHashSearch: Use `#key?` instead of `#keys.include?`.

    ```rb
                         j.keys.include?("payment")
    ```

  * **Line # 166 - convention:** RSpec/ExampleLength: Example has too many lines. [12/5]

    ```rb
            it "ignores param" do ...
    ```

  * **Line # 166 - convention:** RSpec/MultipleExpectations: Example has too many expectations [8/1].
  ...
  * **Line # 210 - convention:** Layout/LineLength: Line is too long. [124/120]

    ```rb
                expect(subject).to all(include(status: "active").or(include(status: "noshow")).or(include(status: "cancelled")))
    ```

  * **Line # 213 - convention:** RSpec/NamedSubject: Name your test subject if you need to reference it explicitly.
  ...
  * **Line # 328 - convention:** Style/RedundantArgument: Argument " " is redundant because it is implied by default.

    ```rb
              let(:query) { reservations.sample.fullname.split(" ").sample }
    ```

  * **Line # 344 - convention:** Style/RedundantArgument: Argument " " is redundant because it is implied by default.

    ```rb
              let(:query) { reservations.sample.notes.split(" ").sample }
    ```

  * **Line # 353 - convention:** RSpec/LetSetup: Do not use `let!` to setup objects not referenced in tests.

    ```rb
            let!(:reservations) do
    ```

  * **Line # 356 - convention:** Rails/TimeZone: Do not use `Time.now` without zone. Use one of `Time.zone.now`, `Time.current`, `Time.now.in_time_zone`, `Time.now.utc`, `Time.now.getlocal`, `Time.now.xmlschema`, `Time.now.iso8601`, `Time.now.jisx0301`, `Time.now.rfc3339`, `Time.now.httpdate`, `Time.now.to_i`, `Time.now.to_f` instead.

    ```rb
                create(:reservation, status: :active, datetime: Time.now),
    ```

  * **Line # 370 - convention:** Rails/TimeZone: Do not use `Time.now` without zone. Use one of `Time.zone.now`, `Time.current`, `Time.now.in_time_zone`, `Time.now.utc`, `Time.now.getlocal`, `Time.now.xmlschema`, `Time.now.iso8601`, `Time.now.jisx0301`, `Time.now.rfc3339`, `Time.now.httpdate`, `Time.now.to_i`, `Time.now.to_f` instead.

    ```rb
              it { expect(parsed_response_body[:items][0][:datetime].to_date).to eq Time.now.to_date }
    ```

  * **Line # 376 - convention:** Rails/Date: Do not use `Date.today` without zone. Use `Time.zone.today` instead.

    ```rb
              before { req(date: Date.today.to_date) }
    ```

  * **Line # 380 - convention:** Rails/TimeZone: Do not use `Time.now` without zone. Use one of `Time.zone.now`, `Time.current`, `Time.now.in_time_zone`, `Time.now.utc`, `Time.now.getlocal`, `Time.now.xmlschema`, `Time.now.iso8601`, `Time.now.jisx0301`, `Time.now.rfc3339`, `Time.now.httpdate`, `Time.now.to_i`, `Time.now.to_f` instead.

    ```rb
              it { expect(parsed_response_body[:items][0][:datetime].to_date).to eq Time.now.to_date }
    ```

  * **Line # 414 - convention:** Layout/LineLength: Line is too long. [127/120]

    ```rb
            context "when filtering by {date_from: 1.day.from_now.end_of_day.to_datetime.to_s, date_to: 1.day.from_now.to_date}" do
    ```

  * **Line # 430 - convention:** Layout/LineLength: Line is too long. [135/120]

    ```rb
            context "when filtering by {datetime_from: 1.day.from_now.end_of_day.to_datetime.to_s, datetime_to: 1.day.from_now.to_date}" do
    ```

  * **Line # 526 - convention:** Performance/CollectionLiteralInLoop: Avoid immutable Array literals in loops. It is better to extract it into a local variable or a constant.

    ```rb
            %w[order_by_direction order_by_order].each do |order_by_order_name|
    ```

  * **Line # 545 - convention:** Performance/CollectionLiteralInLoop: Avoid immutable Array literals in loops. It is better to extract it into a local variable or a constant.

    ```rb
            %w[attribute column field by].each do |attribute_alias|
    ```

  * **Line # 546 - convention:** Layout/LineLength: Line is too long. [132/120]

    ```rb
              context "when ordering with {order_by: { #{attribute_alias.inspect}: 'datetime', #{direction_alias.inspect}: 'DESC' }}" do
    ```

  * **Line # 554 - convention:** RSpec/MultipleExpectations: Example has too many expectations [5/1].
  ...
  * **Line # 649 - convention:** RSpec/ExampleLength: Example has too many lines. [8/5]

    ```rb
            it do ...
    ```

  * **Line # 649 - convention:** RSpec/MultipleExpectations: Example has too many expectations [7/1].

    ```rb
            it do
    ```

  * **Line # 685 - convention:** RSpec/ExampleLength: Example has too many lines. [19/5]

    ```rb
            it "returns reservation info" do ...
    ```

  * **Line # 685 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
            it "returns reservation info" do
    ```

  * **Line # 712 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].

    ```rb
            it "returns 200" do
    ```

  * **Line # 721 - convention:** Performance/CollectionLiteralInLoop: Avoid immutable Array literals in loops. It is better to extract it into a local variable or a constant.

    ```rb
            ["2024-10-12 19:00", "2024-12-25 21:00"].each do |datetime|
    ```

  * **Line # 722 - convention:** Performance/CollectionLiteralInLoop: Avoid immutable Array literals in loops. It is better to extract it into a local variable or a constant.

    ```rb
              [1, 2, 3].each do |adults|
    ```

  * **Line # 723 - convention:** Layout/LineLength: Line is too long. [121/120]

    ```rb
                context "when providing {fullname: #{fullname.inspect}, datetime: #{datetime.inspect}, adults: #{adults}}" do
    ```

  * **Line # 728 - convention:** RSpec/ExampleLength: Example has too many lines. [6/5]

    ```rb
                  it "returns provided info" do ...
    ```

  * **Line # 728 - convention:** RSpec/MultipleExpectations: Example has too many expectations [5/1].

    ```rb
                  it "returns provided info" do
    ```

  * **Line # 732 - convention:** Style/RedundantArgument: Argument " " is redundant because it is implied by default.

    ```rb
                    expect(parsed_response_body.dig(:item, :datetime)).to include(datetime.split(" ").first)
    ```

  * **Line # 733 - convention:** Style/RedundantArgument: Argument " " is redundant because it is implied by default.

    ```rb
                    expect(parsed_response_body.dig(:item, :datetime)).to include(datetime.split(" ").last)
    ```

  * **Line # 738 - convention:** Performance/CollectionLiteralInLoop: Avoid immutable Array literals in loops. It is better to extract it into a local variable or a constant.

    ```rb
                [201, "204 fuori"].each do |table|
    ```

  * **Line # 739 - convention:** Performance/CollectionLiteralInLoop: Avoid immutable Array literals in loops. It is better to extract it into a local variable or a constant.

    ```rb
                  ["bambini", "bella vita"].each do |notes|
    ```

  * **Line # 740 - convention:** Performance/CollectionLiteralInLoop: Avoid immutable Array literals in loops. It is better to extract it into a local variable or a constant.

    ```rb
                    ["sa@ba", "gi@gi"].each do |email|
    ```

  * **Line # 741 - convention:** Performance/CollectionLiteralInLoop: Avoid immutable Array literals in loops. It is better to extract it into a local variable or a constant.

    ```rb
                      ["123 333 333", "456 666 666"].each do |phone|
    ```

  * **Line # 742 - convention:** Layout/LineLength: Line is too long. [229/120]
  ...
  * **Line # 745 - convention:** RSpec/ExampleLength: Example has too many lines. [7/5]

    ```rb
                          it "returns provided info" do ...
    ```

  * **Line # 745 - convention:** RSpec/MultipleExpectations: Example has too many expectations [6/1].

    ```rb
                          it "returns provided info" do
    ```

  * **Line # 750 - convention:** Style/RedundantArgument: Argument " " is redundant because it is implied by default.

    ```rb
                            expect(parsed_response_body.dig(:item, :datetime)).to include(datetime.split(" ").first)
    ```

  * **Line # 751 - convention:** Style/RedundantArgument: Argument " " is redundant because it is implied by default.
  ...
  * **Line # 837 - convention:** Style/StringConcatenation: Prefer string interpolation to string concatenation.

    ```rb
            let(:notes) { "Please be kind" + SecureRandom.hex }
    ```

  * **Line # 846 - convention:** Style/StringConcatenation: Prefer string interpolation to string concatenation.

    ```rb
            let(:email) { "giuly@presley" + SecureRandom.hex }
    ```

  * **Line # 929 - convention:** Layout/LineLength: Line is too long. [140/120]

    ```rb
                                                                                                 action: :update_status, id: "2", format: :json)
    ```

  * **Line # 956 - convention:** RSpec/ExampleLength: Example has too many lines. [7/5]

    ```rb
            it do ...
    ```

  * **Line # 956 - convention:** RSpec/MultipleExpectations: Example has too many expectations [7/1].

    ```rb
            it do
    ```

  * **Line # 970 - convention:** RSpec/ExampleLength: Example has too many lines. [7/5]

    ```rb
            it do ...
    ```

  * **Line # 970 - convention:** RSpec/MultipleExpectations: Example has too many expectations [7/1].

    ```rb
            it do
    ```

  * **Line # 984 - convention:** RSpec/ExampleLength: Example has too many lines. [8/5]

    ```rb
            it do ...
    ```

  * **Line # 984 - convention:** RSpec/MultipleExpectations: Example has too many expectations [8/1].

    ```rb
            it do
    ```

  * **Line # 992 - convention:** Rails/TimeZone: Do not use `Time.now` without zone. Use one of `Time.zone.now`, `Time.current`, `Time.now.in_time_zone`, `Time.now.utc`, `Time.now.getlocal`, `Time.now.xmlschema`, `Time.now.iso8601`, `Time.now.jisx0301`, `Time.now.rfc3339`, `Time.now.httpdate`, `Time.now.to_i`, `Time.now.to_f` instead.

    ```rb
              expect(json[:item][:cancelled_at]).to include(Time.now.strftime("%Y-%m-%d"))
    ```

  * **Line # 999 - convention:** RSpec/MultipleExpectations: Example has too many expectations [4/1].

    ```rb
            it do
    ```

  * **Line # 1055 - convention:** RSpec/ExpectChange: Prefer `change(TagInReservation, :count)`.

    ```rb
            it { expect { req }.to change { TagInReservation.count }.by(1) }
    ```

  * **Line # 1056 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { req }.not_to(change { Reservation.count }) }
    ```

  * **Line # 1057 - convention:** RSpec/ExpectChange: Prefer `change(ReservationTag, :count)`.

    ```rb
            it { expect { req }.not_to(change { ReservationTag.count }) }
    ```

  * **Line # 1061 - convention:** RSpec/ExampleLength: Example has too many lines. [6/5]

    ```rb
            it "is successful" do ...
    ```

  * **Line # 1061 - convention:** RSpec/MultipleExpectations: Example has too many expectations [5/1].

    ```rb
            it "is successful" do
    ```

  * **Line # 1070 - convention:** RSpec/MultipleExpectations: Example has too many expectations [4/1].

    ```rb
            it "can try to add twice the tag, will be added just once." do
    ```

  * **Line # 1071 - convention:** RSpec/ExpectChange: Prefer `change(TagInReservation, :count)`.

    ```rb
              expect { req }.to change { TagInReservation.count }.by(1)
    ```

  * **Line # 1073 - convention:** RSpec/ExpectChange: Prefer `change(TagInReservation, :count)`.

    ```rb
              expect { req }.not_to(change { TagInReservation.count })
    ```

  * **Line # 1084 - convention:** RSpec/AnyInstance: Avoid stubbing using `allow_any_instance_of`.

    ```rb
              allow_any_instance_of(TagInReservation).to receive(:valid?).and_return(false)
    ```

  * **Line # 1087 - convention:** RSpec/AnyInstance: Avoid stubbing using `allow_any_instance_of`.

    ```rb
              allow_any_instance_of(TagInReservation).to receive(:errors).and_return(errors)
    ```

  * **Line # 1090 - convention:** RSpec/ExpectChange: Prefer `change(TagInReservation, :count)`.

    ```rb
            it { expect { req }.not_to(change { TagInReservation.count }) }
    ```

  * **Line # 1092 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
            it "returns 422 with message" do
    ```

  * **Line # 1108 - convention:** Layout/LineLength: Line is too long. [136/120]

    ```rb
                                                                                                action: :remove_tag, id: "2", format: :json)
    ```

  * **Line # 1147 - convention:** RSpec/ExpectChange: Prefer `change(TagInReservation, :count)`.

    ```rb
            it { expect { req }.to change { TagInReservation.count }.by(-1) }
    ```

  * **Line # 1148 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { req }.not_to(change { Reservation.count }) }
    ```

  * **Line # 1149 - convention:** RSpec/ExpectChange: Prefer `change(ReservationTag, :count)`.

    ```rb
            it { expect { req }.not_to(change { ReservationTag.count }) }
    ```

  * **Line # 1154 - convention:** RSpec/ExampleLength: Example has too many lines. [6/5]

    ```rb
            it "is successful" do ...
    ```

  * **Line # 1154 - convention:** RSpec/MultipleExpectations: Example has too many expectations [5/1].

    ```rb
            it "is successful" do
    ```

  * **Line # 1163 - convention:** RSpec/MultipleExpectations: Example has too many expectations [4/1].

    ```rb
            it "when removing same tag twice, should be fine." do
    ```

  * **Line # 1179 - convention:** RSpec/ExpectChange: Prefer `change(TagInReservation, :count)`.

    ```rb
            it { expect { req }.to change { TagInReservation.count }.by(-1) }
    ```

  * **Line # 1180 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { req }.not_to(change { Reservation.count }) }
    ```

  * **Line # 1181 - convention:** RSpec/ExpectChange: Prefer `change(ReservationTag, :count)`.

    ```rb
            it { expect { req }.not_to(change { ReservationTag.count }) }
    ```

  * **Line # 1186 - convention:** RSpec/ExampleLength: Example has too many lines. [6/5]

    ```rb
            it "is successful" do ...
    ```

  * **Line # 1186 - convention:** RSpec/MultipleExpectations: Example has too many expectations [5/1].

    ```rb
            it "is successful" do
    ```

  * **Line # 1195 - convention:** RSpec/MultipleExpectations: Example has too many expectations [4/1].

    ```rb
            it "when removing same tag twice, should be fine." do
    ```

  * **Line # 1223 - warning:** Lint/UnderscorePrefixedVariableName: Do not use prefix `_` for a variable that is used.

    ```rb
        def req(_params = params)
    ```

  * **Line # 1251 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].

    ```rb
            it "is successful" do
    ```

  * **Line # 1259 - convention:** RSpec/AnyInstance: Avoid stubbing using `allow_any_instance_of`.

    ```rb
                allow_any_instance_of(Hash).to receive(:dig!).and_call_original
    ```

  * **Line # 1267 - convention:** RSpec/ExpectChange: Prefer `change(Log::DeliveredEmail, :count)`.

    ```rb
              it { expect { req }.to change { Log::DeliveredEmail.count }.by(1) }
    ```

  * **Line # 1269 - convention:** RSpec/ExpectChange: Prefer `change(Log::ImagePixel, :count)`.

    ```rb
              it { expect { req }.to change { Log::ImagePixel.count }.by(1) }
    ```

  * **Line # 1271 - convention:** RSpec/MultipleExpectations: Example has too many expectations [4/1].

    ```rb
              it "is successful" do
    ```

  * **Line # 1279 - convention:** RSpec/ExampleLength: Example has too many lines. [9/5]

    ```rb
              it "last delivered email should have the correct reservation" do ...
    ```

  * **Line # 1279 - convention:** RSpec/MultipleExpectations: Example has too many expectations [6/1].

    ```rb
              it "last delivered email should have the correct reservation" do
    ```

  * **Line # 1298 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].

    ```rb
              it "is successful" do
    ```

  * **Line # 1308 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
              it "returns delivery details" do
    ```

  * **Line # 1314 - convention:** RSpec/ExampleLength: Example has too many lines. [7/5]

    ```rb
              it do ...
    ```

  * **Line # 1314 - convention:** RSpec/MultipleExpectations: Example has too many expectations [5/1].

    ```rb
              it do
    ```

  * **Line # 1374 - convention:** RSpec/LetSetup: Do not use `let!` to setup objects not referenced in tests.

    ```rb
            let!(:reservations) do
    ```

  * **Line # 1377 - convention:** Rails/TimeZone: Do not use `Time.now` without zone. Use one of `Time.zone.now`, `Time.current`, `Time.now.in_time_zone`, `Time.now.utc`, `Time.now.getlocal`, `Time.now.xmlschema`, `Time.now.iso8601`, `Time.now.jisx0301`, `Time.now.rfc3339`, `Time.now.httpdate`, `Time.now.to_i`, `Time.now.to_f` instead.

    ```rb
                create(:reservation, status: :active, datetime: Time.now, adults: 2),
    ```

  * **Line # 1384 - convention:** RSpec/LetSetup: Do not use `let!` to setup objects not referenced in tests.

    ```rb
            let!(:reservation_turns) do
    ```

  * **Line # 1391 - convention:** RSpec/LetSetup: Do not use `let!` to setup objects not referenced in tests.

    ```rb
              let!(:reservation_turns) do
    ```

  * **Line # 1398 - convention:** RSpec/LetSetup: Do not use `let!` to setup objects not referenced in tests.

    ```rb
              let!(:reservations) do
    ```

  * **Line # 1470 - convention:** Rails/Date: Do not use `Date.today` without zone. Use `Time.zone.today` instead.

    ```rb
              before { req(date: Date.today.to_date) }
    ```

  * **Line # 1573 - convention:** Layout/LineLength: Line is too long. [133/120]

    ```rb
              expect(file.sheet("Prenotazioni").column(col_index("payment_hpp_url"))).to include(*ReservationPayment.all.pluck(:hpp_url))
    ```

  * **Line # 1600 - convention:** Style/RedundantInterpolation: Prefer `to_s` over string interpolation.

    ```rb
          let(:query) { "#{secret[1..15]}" }
    ```

  * **Line # 1664 - convention:** RSpec/MissingExampleGroupArgument: The first argument to `context` should not be empty.

    ```rb
          context do ...
    ```

  * **Line # 1674 - convention:** RSpec/MissingExampleGroupArgument: The first argument to `context` should not be empty.

    ```rb
          context do ...
    ```

  * **Line # 1684 - convention:** RSpec/MissingExampleGroupArgument: The first argument to `context` should not be empty.

    ```rb
          context do ...
    ```

  * **Line # 1715 - convention:** RSpec/InstanceVariable: Avoid instance variables - use let, a method call, or a local variable (if possible).

    ```rb
            it { expect(col_values("id")).to contain_exactly(@middle.id) }
    ```

  * **Line # 1724 - convention:** RSpec/InstanceVariable: Avoid instance variables - use let, a method call, or a local variable (if possible).
  ...
  * **Line # 1733 - convention:** RSpec/InstanceVariable: Avoid instance variables - use let, a method call, or a local variable (if possible).
...
### spec/controllers/v1/menu/categories_controller_spec.rb - (136 offenses)
  ...
  * **Line # 560 - warning:** Lint/UselessAssignment: Useless assignment to variable - `items`.
  ...
  * **Line # 635 - warning:** Lint/UselessAssignment: Useless assignment to variable - `items`.
  ...
  * **Line # 678 - warning:** Lint/UselessAssignment: Useless assignment to variable - `items`.
...
### spec/controllers/v1/reservations_controller/reservations_controller.cancel_spec.rb - (22 offenses)
  ...
  * **Line # 34 - convention:** Layout/LineLength: Line is too long. [125/120]

    ```rb
                       "#{Config.app.dig!(:nexi_api_url)}/#{Config.app.dig!(:nexi_refund_payment_path)}").to_return do |_request|
    ```

  * **Line # 54 - convention:** RSpec/ExpectChange: Prefer `change(Log::DeliveredEmail, :count)`.

    ```rb
          it { expect { req }.to change { Log::DeliveredEmail.count }.by(1) }
    ```

  * **Line # 62 - convention:** RSpec/LetSetup: Do not use `let!` to setup objects not referenced in tests.

    ```rb
              let!(:payment) { create(:reservation_payment, status: payment_status, reservation:) }
    ```

  * **Line # 64 - convention:** RSpec/ExpectChange: Prefer `change(ReservationPayment, :count)`.

    ```rb
              it  { expect { req }.not_to(change { ReservationPayment.count }) }
    ```

  * **Line # 65 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
              it  { expect { req }.not_to(change { Reservation.count }) }
    ```

  * **Line # 67 - convention:** RSpec/ExpectChange: Prefer `change(Log::DeliveredEmail, :count)`.

    ```rb
              it { expect { req }.to change { Log::DeliveredEmail.count }.by(1) }
    ```

  * **Line # 91 - convention:** RSpec/ExpectChange: Prefer `change(ReservationPayment, :count)`.

    ```rb
            it  { expect { req }.not_to(change { ReservationPayment.count }) }
    ```

  * **Line # 92 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it  { expect { req }.not_to(change { Reservation.count }) }
    ```

  * **Line # 110 - convention:** RSpec/ExpectChange: Prefer `change(ReservationPayment, :count)`.

    ```rb
              it  { expect { req }.not_to(change { ReservationPayment.count }) }
    ```

  * **Line # 111 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
              it  { expect { req }.not_to(change { Reservation.count }) }
    ```

  * **Line # 126 - convention:** RSpec/ExpectChange: Prefer `change(ReservationPayment, :count)`.

    ```rb
            it  { expect { req }.not_to(change { ReservationPayment.count }) }
    ```

  * **Line # 127 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it  { expect { req }.not_to(change { Reservation.count }) }
    ```

  * **Line # 146 - convention:** Performance/CollectionLiteralInLoop: Avoid immutable Array literals in loops. It is better to extract it into a local variable or a constant.

    ```rb
            [ ...
    ```

  * **Line # 179 - convention:** Performance/CollectionLiteralInLoop: Avoid immutable Array literals in loops. It is better to extract it into a local variable or a constant.

    ```rb
            [ ...
    ```

  * **Line # 211 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].

    ```rb
            it do
    ```

  * **Line # 243 - convention:** RSpec/AnyInstance: Avoid stubbing using `allow_any_instance_of`.

    ```rb
              allow_any_instance_of(Reservation).to receive(:cancelled!).and_return(false)
    ```

  * **Line # 246 - convention:** RSpec/AnyInstance: Avoid stubbing using `allow_any_instance_of`.
...
### spec/controllers/v1/reservations_controller/reservations_controller.create_spec.rb - (196 offenses)
  * **Line # 10 - convention:** RSpec/SharedContext: Use `shared_examples` when you don't define context.

    ```rb
    RSpec.shared_context "SUCCESSFUL V1::ReservationsController POST #create" do |options = {}|
    ```

  * **Line # 45 - convention:** RSpec/ExpectChange: Prefer `change(Nexi::HttpRequest, :count)`.

    ```rb
        it { expect { req }.to(change { Nexi::HttpRequest.count }.by(1)) }
    ```

  * **Line # 67 - convention:** RSpec/Rails/InferredSpecType: Remove redundant spec type.
  ...
  * **Line # 126 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
          it { expect { req }.to change { Reservation.count }.by(1) }
    ```

  * **Line # 128 - convention:** RSpec/MissingExampleGroupArgument: The first argument to `context` should not be empty.

    ```rb
          context do ...
    ```

  * **Line # 140 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
          it { expect { req }.to change { Reservation.count }.by(1) }
    ```

  * **Line # 142 - convention:** RSpec/MissingExampleGroupArgument: The first argument to `context` should not be empty.

    ```rb
          context do ...
    ```

  * **Line # 154 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
          it { expect { req }.not_to(change { Reservation.count }) }
    ```

  * **Line # 156 - convention:** RSpec/MissingExampleGroupArgument: The first argument to `context` should not be empty.

    ```rb
          context do ...
    ```

  * **Line # 172 - convention:** Layout/LineLength: Line is too long. [129/120]

    ```rb
          context "when providing a #{scenario[:name]} date, should create a reservation for turn with weekday=#{scenario[:wday]}" do
    ```

  * **Line # 174 - convention:** RSpec/LetSetup: Do not use `let!` to setup objects not referenced in tests.

    ```rb
            let!(:turn) do
    ```

  * **Line # 184 - convention:** RSpec/LetSetup: Do not use `let!` to setup objects not referenced in tests.

    ```rb
          let!(:turn) do
    ```

  * **Line # 203 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { doit }.to(change { Reservation.count }) }
    ```

  * **Line # 220 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
              it { expect { doit }.to(change { Reservation.count }) }
    ```

  * **Line # 238 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
              it { expect { doit }.not_to(change { Reservation.count }) }
    ```

  * **Line # 254 - convention:** RSpec/LetSetup: Do not use `let!` to setup objects not referenced in tests.

    ```rb
          let!(:turn) do
    ```

  * **Line # 325 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].
  ...
  * **Line # 358 - convention:** Layout/LineLength: Line is too long. [122/120]

    ```rb
        context "when a 'weekly' Holiday exists (closed only on the morning while the reservation is made for the evening)" do
    ```

  * **Line # 359 - convention:** RSpec/LetSetup: Do not use `let!` to setup objects not referenced in tests.
  ...
  * **Line # 397 - convention:** RSpec/LetSetup: Do not use `let!` to setup objects not referenced in tests.

    ```rb
          let!(:holiday) do
    ```

  * **Line # 410 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].

    ```rb
          it do
    ```

  * **Line # 420 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
          it { expect { req }.not_to(change { Reservation.count }) }
    ```

  * **Line # 422 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].

    ```rb
          it do
    ```

  * **Line # 433 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { req }.to(change { Reservation.count }.by(1)) }
    ```

  * **Line # 436 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].

    ```rb
            it do
    ```

  * **Line # 448 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { req }.not_to(change { Reservation.count }) }
    ```

  * **Line # 450 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].

    ```rb
            it do
    ```

  * **Line # 466 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { req }.to change { Reservation.count }.by(1) }
    ```

  * **Line # 468 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
            it do
    ```

  * **Line # 479 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { req }.to change { Reservation.count }.by(1) }
    ```

  * **Line # 481 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
            it do
    ```

  * **Line # 503 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { req }.to change { Reservation.count }.by(1) }
    ```

  * **Line # 505 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
            it do
    ```

  * **Line # 516 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { req }.to change { Reservation.count }.by(1) }
    ```

  * **Line # 518 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
            it do
    ```

  * **Line # 530 - convention:** Layout/LineLength: Line is too long. [127/120]

    ```rb
                         "#{Config.app.dig!(:nexi_api_url)}/#{Config.app.dig!(:nexi_simple_payment_path)}").to_return do |_request|
    ```

  * **Line # 533 - convention:** Rails/FilePath: Prefer `Rails.root.join('path/to')`.

    ```rb
                body: File.read(Rails.root.join("spec", "fixtures", "nexi-error-page.html"))
    ```

  * **Line # 555 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { req }.not_to(change { Reservation.count }) }
    ```

  * **Line # 556 - convention:** RSpec/ExpectChange: Prefer `change(ReservationPayment, :count)`.

    ```rb
            it { expect { req }.not_to(change { ReservationPayment.count }) }
    ```

  * **Line # 557 - convention:** RSpec/ExpectChange: Prefer `change(Nexi::HttpRequest, :count)`.
  ...
  * **Line # 571 - convention:** Layout/LineLength: Line is too long. [127/120]

    ```rb
                         "#{Config.app.dig!(:nexi_api_url)}/#{Config.app.dig!(:nexi_simple_payment_path)}").to_return do |_request|
    ```

  * **Line # 596 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { req }.not_to(change { Reservation.count }) }
    ```

  * **Line # 597 - convention:** RSpec/ExpectChange: Prefer `change(ReservationPayment, :count)`.

    ```rb
            it { expect { req }.not_to(change { ReservationPayment.count }) }
    ```

  * **Line # 598 - convention:** RSpec/ExpectChange: Prefer `change(Nexi::HttpRequest, :count)`.

    ```rb
            it { expect { req }.to(change { Nexi::HttpRequest.count }.by(1)) }
    ```

  * **Line # 601 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].

    ```rb
            it do
    ```

  * **Line # 612 - convention:** Layout/LineLength: Line is too long. [127/120]

    ```rb
                         "#{Config.app.dig!(:nexi_api_url)}/#{Config.app.dig!(:nexi_simple_payment_path)}").to_return do |_request|
    ```

  * **Line # 620 - convention:** Rails/FilePath: Prefer `Rails.root.join('path/to')`.

    ```rb
            File.read(Rails.root.join("spec", "fixtures", "nexi-unauthorized-page.html"))
    ```

  * **Line # 634 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { req }.not_to(change { Reservation.count }) }
    ```

  * **Line # 635 - convention:** RSpec/ExpectChange: Prefer `change(ReservationPayment, :count)`.

    ```rb
            it { expect { req }.not_to(change { ReservationPayment.count }) }
    ```

  * **Line # 636 - convention:** RSpec/ExpectChange: Prefer `change(Nexi::HttpRequest, :count)`.

    ```rb
            it { expect { req }.to(change { Nexi::HttpRequest.count }.by(1)) }
    ```

  * **Line # 659 - convention:** Layout/LineLength: Line is too long. [127/120]

    ```rb
                         "#{Config.app.dig!(:nexi_api_url)}/#{Config.app.dig!(:nexi_simple_payment_path)}").to_return do |_request|
    ```

  * **Line # 667 - convention:** Rails/FilePath: Prefer `Rails.root.join('path/to')`.

    ```rb
            File.read(Rails.root.join("spec", "fixtures", "nexi-simple-payment-success-page.html"))
    ```

  * **Line # 679 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { req }.to(change { Reservation.count }.by(1)) }
    ```

  * **Line # 681 - convention:** RSpec/ExpectChange: Prefer `change(ReservationPayment, :count)`.

    ```rb
            it { expect { req }.to(change { ReservationPayment.count }.by(1)) }
    ```

  * **Line # 682 - convention:** RSpec/ExpectChange: Prefer `change(Nexi::HttpRequest, :count)`.

    ```rb
            it { expect { req }.to(change { Nexi::HttpRequest.count }.by(1)) }
    ```

  * **Line # 700 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { req }.to(change { Reservation.count }.by(1)) }
    ```

  * **Line # 701 - convention:** RSpec/ExpectChange: Prefer `change(ReservationPayment, :count)`.

    ```rb
            it { expect { req }.to(change { ReservationPayment.count }.by(1)) }
    ```

  * **Line # 702 - convention:** RSpec/ExpectChange: Prefer `change(Nexi::HttpRequest, :count)`.

    ```rb
            it { expect { req }.to(change { Nexi::HttpRequest.count }.by(1)) }
    ```

  * **Line # 723 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { req }.to(change { Reservation.count }.by(1)) }
    ```

  * **Line # 724 - convention:** RSpec/ExpectChange: Prefer `change(ReservationPayment, :count)`.

    ```rb
            it { expect { req }.to(change { ReservationPayment.count }.by(1)) }
    ```

  * **Line # 725 - convention:** RSpec/ExpectChange: Prefer `change(Nexi::HttpRequest, :count)`.

    ```rb
            it { expect { req }.to(change { Nexi::HttpRequest.count }.by(1)) }
    ```

  * **Line # 728 - convention:** Layout/TrailingWhitespace: Trailing whitespace detected.

    ```rb
              req 
    ```

  * **Line # 729 - convention:** Layout/LineLength: Line is too long. [126/120]

    ```rb
              expect(ActionMailer::MailDeliveryJob).to have_been_enqueued.with("ReservationMailer", "payment_required_to_confirm",
    ```

  * **Line # 730 - convention:** Layout/LineLength: Line is too long. [128/120]

    ```rb
                                                                               "deliver_now", params: anything, args: anything).once
    ```

  * **Line # 733 - convention:** RSpec/ExampleLength: Example has too many lines. [23/5]

    ```rb
            it do ...
    ```

  * **Line # 733 - convention:** RSpec/MultipleExpectations: Example has too many expectations [22/1].

    ```rb
            it do
    ```

  * **Line # 756 - convention:** Layout/LineLength: Line is too long. [144/120]
  ...
  * **Line # 759 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].

    ```rb
            it do
    ```

  * **Line # 772 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
                it { expect { req }.to(change { Reservation.count }.by(1)) }
    ```

  * **Line # 775 - convention:** RSpec/ExpectChange: Prefer `change(Nexi::HttpRequest, :count)`.

    ```rb
                it { expect { req }.to(change { Nexi::HttpRequest.count }.by(1)) }
    ```

  * **Line # 777 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].

    ```rb
                it do
    ```

  * **Line # 783 - convention:** RSpec/MultipleExpectations: Example has too many expectations [4/1].

    ```rb
                it do
    ```

  * **Line # 793 - convention:** Style/SingleArgumentDig: Use `Nexi::HttpRequest.last.request_body["languageId"]` instead of `Nexi::HttpRequest.last.request_body.dig("languageId")`.

    ```rb
                  expect(Nexi::HttpRequest.last.request_body.dig("languageId")).to eq(scenario[:code])
    ```

  * **Line # 828 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
                it { expect { req }.to(change { Reservation.count }.by(1)) }
    ```

  * **Line # 830 - convention:** RSpec/ExpectChange: Prefer `change(ReservationPayment, :count)`.

    ```rb
                it { expect { req }.to(change { ReservationPayment.count }.by(1)) }
    ```

  * **Line # 836 - convention:** RSpec/ExpectInHook: Do not use `expect` in `before` hook

    ```rb
                    expect(Reservation.where(table_type:).pluck(:adults, :children).flatten.sum).to eq 15
    ```

  * **Line # 837 - convention:** RSpec/ExpectInHook: Do not use `expect` in `before` hook

    ```rb
                    expect(Reservation.pluck(:adults, :children).flatten.sum).to eq 15
    ```

  * **Line # 840 - convention:** RSpec/MultipleExpectations: Example has too many expectations [4/1].

    ```rb
                  it do
    ```

  * **Line # 868 - convention:** RSpec/IndexedLet: This `let` statement uses index in its name. Please give it a meaningful name.

    ```rb
                      let(:datetime_18) { "#{date.to_date} 18:00" }
    ```

  * **Line # 868 - convention:** Naming/VariableNumber: Use normalcase for symbol numbers.

    ```rb
                      let(:datetime_18) { "#{date.to_date} 18:00" }
    ```

  * **Line # 869 - convention:** RSpec/IndexedLet: This `let` statement uses index in its name. Please give it a meaningful name.

    ```rb
                      let(:datetime_21) { "#{date.to_date} 21:00" }
    ```

  * **Line # 869 - convention:** Naming/VariableNumber: Use normalcase for symbol numbers.

    ```rb
                      let(:datetime_21) { "#{date.to_date} 21:00" }
    ```

  * **Line # 872 - convention:** RSpec/ExpectInHook: Do not use `expect` in `before` hook

    ```rb
                        expect do
    ```

  * **Line # 879 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
                      it do
    ```

  * **Line # 907 - convention:** RSpec/IndexedLet: This `let` statement uses index in its name. Please give it a meaningful name.

    ```rb
                      let(:datetime_18) { "#{date.to_date} 18:00" }
    ```

  * **Line # 907 - convention:** Naming/VariableNumber: Use normalcase for symbol numbers.

    ```rb
                      let(:datetime_18) { "#{date.to_date} 18:00" }
    ```

  * **Line # 908 - convention:** RSpec/IndexedLet: This `let` statement uses index in its name. Please give it a meaningful name.

    ```rb
                      let(:datetime_1830) { "#{date.to_date} 18:30" }
    ```

  * **Line # 908 - convention:** Naming/VariableNumber: Use normalcase for symbol numbers.

    ```rb
                      let(:datetime_1830) { "#{date.to_date} 18:30" }
    ```

  * **Line # 909 - convention:** RSpec/IndexedLet: This `let` statement uses index in its name. Please give it a meaningful name.

    ```rb
                      let(:datetime_21) { "#{date.to_date} 21:00" }
    ```

  * **Line # 909 - convention:** Naming/VariableNumber: Use normalcase for symbol numbers.
  ...
  * **Line # 912 - convention:** RSpec/LetSetup: Do not use `let!` to setup objects not referenced in tests.

    ```rb
                      let!(:turn) do
    ```

  * **Line # 917 - convention:** RSpec/LetSetup: Do not use `let!` to setup objects not referenced in tests.

    ```rb
                      let!(:turn2) do
    ```

  * **Line # 932 - convention:** Performance/CollectionLiteralInLoop: Avoid immutable Array literals in loops. It is better to extract it into a local variable or a constant.

    ```rb
                        Reservation.all.each { |r| r.update!(status: %w[deleted cancelled].sample) }
    ```

  * **Line # 941 - convention:** RSpec/ExpectInHook: Do not use `expect` in `before` hook

    ```rb
                        expect do
    ```

  * **Line # 945 - convention:** RSpec/ExpectInHook: Do not use `expect` in `before` hook

    ```rb
                        expect do
    ```

  * **Line # 956 - convention:** Layout/LineLength: Line is too long. [124/120]

    ```rb
              context "when base payment requires an authorization, if table_type is specified will create a payment instead" do
    ```

  * **Line # 957 - convention:** RSpec/EmptyLineAfterHook: Add an empty line after `before`.

    ```rb
                before { group.update!(preorder_type: :nexi_authorization) }
    ```

  * **Line # 965 - convention:** Layout/LineLength: Line is too long. [139/120]
  ...
  * **Line # 974 - convention:** RSpec/EmptyLineAfterHook: Add an empty line after `before`.

    ```rb
                before { group.update!(preorder_type: :nexi_payment) }
    ```

  * **Line # 974 - convention:** Layout/LineLength: Line is too long. [123/120]

    ```rb
              context "when base payment requires a payment, if table_type is specified will create a payment (as expected)" do
    ```

  * **Line # 981 - convention:** RSpec/EmptyLineAfterHook: Add an empty line after `before`.

    ```rb
                before { group.update!(preorder_type: :nexi_payment) }
    ```

  * **Line # 982 - convention:** Layout/LineLength: Line is too long. [125/120]

    ```rb
              context "when base payment requires a payment, if table_type is not present will create a payment (as expected)" do
    ```

  * **Line # 1007 - convention:** RSpec/ExpectChange: Prefer `change(ReservationPayment, :count)`.

    ```rb
                it { expect { req }.not_to(change { ReservationPayment.count }) }
    ```

  * **Line # 1015 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
                it do
    ```

  * **Line # 1022 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
                it { expect { req }.not_to(change { Reservation.count }) }
    ```

  * **Line # 1023 - convention:** RSpec/ExpectChange: Prefer `change(ReservationPayment, :count)`.

    ```rb
                it { expect { req }.not_to(change { ReservationPayment.count }) }
    ```

  * **Line # 1029 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
                it do
    ```

  * **Line # 1036 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
                it { expect { req }.not_to(change { Reservation.count }) }
    ```

  * **Line # 1037 - convention:** RSpec/ExpectChange: Prefer `change(ReservationPayment, :count)`.

    ```rb
                it { expect { req }.not_to(change { ReservationPayment.count }) }
    ```

  * **Line # 1043 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
                it do
    ```

  * **Line # 1050 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
                it { expect { req }.not_to(change { Reservation.count }) }
    ```

  * **Line # 1051 - convention:** RSpec/ExpectChange: Prefer `change(ReservationPayment, :count)`.

    ```rb
                it { expect { req }.not_to(change { ReservationPayment.count }) }
    ```

  * **Line # 1086 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
                it { expect { req }.to(change { Reservation.count }.by(1)) }
    ```

  * **Line # 1087 - convention:** RSpec/ExpectChange: Prefer `change(ReservationPayment, :count)`.

    ```rb
                it { expect { req }.to(change { ReservationPayment.count }.by(1)) }
    ```

  * **Line # 1088 - convention:** RSpec/ExpectChange: Prefer `change(Nexi::HttpRequest, :count)`.

    ```rb
                it { expect { req }.to(change { Nexi::HttpRequest.count }.by(1)) }
    ```

  * **Line # 1090 - convention:** RSpec/ExampleLength: Example has too many lines. [8/5]

    ```rb
                it do ...
    ```

  * **Line # 1090 - convention:** RSpec/MultipleExpectations: Example has too many expectations [7/1].

    ```rb
                it do
    ```

  * **Line # 1101 - convention:** RSpec/ExampleLength: Example has too many lines. [6/5]

    ```rb
                it do ...
    ```

  * **Line # 1101 - convention:** RSpec/MultipleExpectations: Example has too many expectations [5/1].

    ```rb
                it do
    ```

  * **Line # 1110 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].

    ```rb
                it do
    ```

  * **Line # 1127 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
                it { expect { req }.to(change { Reservation.count }.by(1)) }
    ```

  * **Line # 1128 - convention:** RSpec/ExpectChange: Prefer `change(ReservationPayment, :count)`.

    ```rb
                it { expect { req }.not_to(change { ReservationPayment.count }) }
    ```

  * **Line # 1129 - convention:** RSpec/ExpectChange: Prefer `change(Nexi::HttpRequest, :count)`.

    ```rb
                it { expect { req }.not_to(change { Nexi::HttpRequest.count }) }
    ```

  * **Line # 1141 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].

    ```rb
                it do
    ```

  * **Line # 1161 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].

    ```rb
          it "creates a reservation" do
    ```

  * **Line # 1170 - convention:** Layout/LineLength: Line is too long. [121/120]

    ```rb
                                                                             "deliver_now", params: anything, args: anything)
    ```

  * **Line # 1191 - convention:** RSpec/LetSetup: Do not use `let!` to setup objects not referenced in tests.

    ```rb
            let!(:turn) do
    ```

  * **Line # 1196 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
            it "returns 422" do
    ```

  * **Line # 1207 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
            it "returns 422" do
    ```

  * **Line # 1218 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { req }.to(change { Reservation.count }.by(1)) }
    ```

  * **Line # 1235 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { req }.to(change { Reservation.count }.by(1)) }
    ```

  * **Line # 1262 - convention:** RSpec/MultipleExpectations: Example has too many expectations [4/1].

    ```rb
                it "returns 422" do
    ```

  * **Line # 1286 - convention:** RSpec/MultipleExpectations: Example has too many expectations [4/1].

    ```rb
                it "returns 422" do
    ```

  * **Line # 1301 - convention:** RSpec/MultipleExpectations: Example has too many expectations [4/1].

    ```rb
            it "returns 422" do
    ```

  * **Line # 1314 - convention:** RSpec/MultipleExpectations: Example has too many expectations [4/1].

    ```rb
            it "returns 422" do
    ```

  * **Line # 1326 - convention:** RSpec/MultipleExpectations: Example has too many expectations [4/1].

    ```rb
            it "returns 422" do
    ```

  * **Line # 1339 - convention:** RSpec/MultipleExpectations: Example has too many expectations [4/1].

    ```rb
            it "returns 422" do
    ```

  * **Line # 1351 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
            it "returns 422" do
    ```

  * **Line # 1362 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
            it "returns 422" do
    ```

  * **Line # 1373 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
            it "returns 422" do
    ```

  * **Line # 1384 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
            it "returns 422" do
    ```

  * **Line # 1395 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
            it "returns 422" do
    ```

  * **Line # 1406 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
            it "returns 422" do
    ```

  * **Line # 1417 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { req }.to change { Reservation.count }.by(1) }
    ```

  * **Line # 1419 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].

    ```rb
            it "returns 200 and create record" do
    ```

  * **Line # 1429 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { req }.to change { Reservation.count }.by(1) }
    ```

  * **Line # 1431 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].

    ```rb
            it "returns 200 and create record" do
    ```

  * **Line # 1441 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { req }.to change { Reservation.count }.by(1) }
    ```

  * **Line # 1443 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].

    ```rb
            it "returns 200 and create record" do
    ```

  * **Line # 1450 - convention:** Layout/LineLength: Line is too long. [129/120]

    ```rb
          context 'should create a reservation with "<firstname> <lastname>" as fullname and save the detail in the "other" field' do
    ```

  * **Line # 1454 - convention:** RSpec/MultipleExpectations: Example has too many expectations [4/1].

    ```rb
            it "creates a reservation" do
    ```

  * **Line # 1467 - convention:** RSpec/MultipleExpectations: Example has too many expectations [4/1].

    ```rb
            it "creates a reservation" do
    ```

  * **Line # 1480 - convention:** RSpec/MultipleExpectations: Example has too many expectations [4/1].

    ```rb
            it "creates a reservation" do
    ```

  * **Line # 1489 - convention:** RSpec/RepeatedExampleGroupBody: Repeated context block body on line(s) [1551]

    ```rb
          context "when email is empty" do ...
    ```

  * **Line # 1489 - convention:** RSpec/RepeatedExampleGroupDescription: Repeated context block description on line(s) [1551]

    ```rb
          context "when email is empty" do ...
    ```

  * **Line # 1492 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { req }.not_to(change { Reservation.count }) }
    ```

  * **Line # 1494 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
            it "returns 422" do
    ```

  * **Line # 1502 - convention:** RSpec/RepeatedExampleGroupBody: Repeated context block body on line(s) [1564]

    ```rb
          context "when email is nil" do ...
    ```

  * **Line # 1502 - convention:** RSpec/RepeatedExampleGroupDescription: Repeated context block description on line(s) [1564]

    ```rb
          context "when email is nil" do ...
    ```

  * **Line # 1505 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { req }.not_to(change { Reservation.count }) }
    ```

  * **Line # 1507 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
            it "returns 422" do
    ```

  * **Line # 1515 - convention:** RSpec/RepeatedExampleGroupBody: Repeated context block body on line(s) [1577]

    ```rb
          context "when email is invalid" do ...
    ```

  * **Line # 1515 - convention:** RSpec/RepeatedExampleGroupDescription: Repeated context block description on line(s) [1577]

    ```rb
          context "when email is invalid" do ...
    ```

  * **Line # 1525 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
                it { expect { req }.not_to(change { Reservation.count }) }
    ```

  * **Line # 1527 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
                it "returns 422" do
    ```

  * **Line # 1538 - convention:** RSpec/LetSetup: Do not use `let!` to setup objects not referenced in tests.

    ```rb
            let!(:reservation) { create(:reservation, datetime:, email:) }
    ```

  * **Line # 1540 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { req }.not_to(change { Reservation.count }) }
    ```

  * **Line # 1542 - convention:** RSpec/MultipleExpectations: Example has too many expectations [4/1].

    ```rb
            it "returns 422" do
    ```

  * **Line # 1551 - convention:** RSpec/RepeatedExampleGroupBody: Repeated context block body on line(s) [1489]

    ```rb
          context "when email is empty" do ...
    ```

  * **Line # 1551 - convention:** RSpec/RepeatedExampleGroupDescription: Repeated context block description on line(s) [1489]

    ```rb
          context "when email is empty" do ...
    ```

  * **Line # 1554 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { req }.not_to(change { Reservation.count }) }
    ```

  * **Line # 1556 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
            it "returns 422" do
    ```

  * **Line # 1564 - convention:** RSpec/RepeatedExampleGroupBody: Repeated context block body on line(s) [1502]

    ```rb
          context "when email is nil" do ...
    ```

  * **Line # 1564 - convention:** RSpec/RepeatedExampleGroupDescription: Repeated context block description on line(s) [1502]
  ...
  * **Line # 1577 - convention:** RSpec/RepeatedExampleGroupBody: Repeated context block body on line(s) [1515]

    ```rb
          context "when email is invalid" do ...
    ```

  * **Line # 1577 - convention:** RSpec/RepeatedExampleGroupDescription: Repeated context block description on line(s) [1515]

    ```rb
          context "when email is invalid" do ...
    ```

  * **Line # 1587 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
                it { expect { req }.not_to(change { Reservation.count }) }
    ```

  * **Line # 1589 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
                it "returns 422" do
    ```

  * **Line # 1602 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { req }.not_to(change { Reservation.count }) }
    ```

  * **Line # 1604 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
            it "returns 422" do
    ```

  * **Line # 1615 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
            it { expect { req }.not_to(change { Reservation.count }) }
    ```

  * **Line # 1617 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
            it "returns 422" do
    ```

  * **Line # 1637 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
                it { expect { req }.not_to(change { Reservation.count }) }
    ```

  * **Line # 1639 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
                it "returns 422" do
    ```

  * **Line # 1672 - convention:** RSpec/ExpectChange: Prefer `change(Reservation, :count)`.

    ```rb
                it { expect { req }.to change { Reservation.count }.by(1) }
    ```

  * **Line # 1680 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].

    ```rb
                it "returns 200" do
    ```

  * **Line # 1687 - convention:** Layout/TrailingEmptyLines: Final newline missing.
...
### spec/requests/v1/admin/preorder_reservation_groups/preorder_reservation_groups.create_spec.rb - (44 offenses)
  ...
  * **Line # 74 - convention:** RSpec/ExampleLength: Example has too many lines. [10/5]

    ```rb
          it do ...
    ```

  * **Line # 118 - convention:** RSpec/RepeatedExampleGroupDescription: Repeated context block description on line(s) [135]

    ```rb
        context "when trying to add same turn to a new group" do ...
    ```

  * **Line # 131 - convention:** RSpec/ExpectChange: Prefer `change(PreorderReservationGroup, :count)`.

    ```rb
          it { expect { req }.not_to(change { PreorderReservationGroup.count }) }
    ```

  * **Line # 132 - convention:** RSpec/ExpectChange: Prefer `change(PreorderReservationGroupsToTurn, :count)`.

    ```rb
          it { expect { req }.not_to(change { PreorderReservationGroupsToTurn.count }) }
    ```

  * **Line # 135 - convention:** RSpec/RepeatedExampleGroupDescription: Repeated context block description on line(s) [118]

    ```rb
        context "when trying to add same turn to a new group" do ...
    ```

  * **Line # 148 - convention:** RSpec/ExpectChange: Prefer `change(PreorderReservationGroup, :count)`.

    ```rb
          it { expect { req }.not_to(change { PreorderReservationGroup.count }) }
    ```

  * **Line # 149 - convention:** RSpec/ExpectChange: Prefer `change(PreorderReservationGroupsToTurn, :count)`.

    ```rb
          it { expect { req }.not_to(change { PreorderReservationGroupsToTurn.count }) }
    ```

  * **Line # 169 - convention:** RSpec/RepeatedExampleGroupDescription: Repeated context block description on line(s) [186]

    ```rb
        context "when trying to add same turn to a new group" do ...
    ```

  * **Line # 182 - convention:** RSpec/ExpectChange: Prefer `change(PreorderReservationGroup, :count)`.

    ```rb
          it { expect { req }.not_to(change { PreorderReservationGroup.count }) }
    ```

  * **Line # 183 - convention:** RSpec/ExpectChange: Prefer `change(PreorderReservationGroupsToTurn, :count)`.

    ```rb
          it { expect { req }.not_to(change { PreorderReservationGroupsToTurn.count }) }
    ```

  * **Line # 186 - convention:** RSpec/RepeatedExampleGroupDescription: Repeated context block description on line(s) [169]

    ```rb
        context "when trying to add same turn to a new group" do ...
    ```

  * **Line # 199 - convention:** RSpec/ExpectChange: Prefer `change(PreorderReservationGroup, :count)`.

    ```rb
          it { expect { req }.not_to(change { PreorderReservationGroup.count }) }
    ```

  * **Line # 200 - convention:** RSpec/ExpectChange: Prefer `change(PreorderReservationGroupsToTurn, :count)`.

    ```rb
          it { expect { req }.not_to(change { PreorderReservationGroupsToTurn.count }) }
    ```

  * **Line # 204 - convention:** Layout/LineLength: Line is too long. [142/120]
  ...
  * **Line # 214 - convention:** RSpec/ExpectChange: Prefer `change(PreorderReservationGroup, :count)`.

    ```rb
        it { expect { req }.to(change { PreorderReservationGroup.count }.by(1)) }
    ```

  * **Line # 215 - convention:** RSpec/ExpectChange: Prefer `change(PreorderReservationDate, :count)`.

    ```rb
        it { expect { req }.to(change { PreorderReservationDate.count }.by(4)) }
    ```

  * **Line # 216 - convention:** RSpec/ExpectChange: Prefer `change(PreorderReservationGroupsToTurn, :count)`.

    ```rb
        it { expect { req }.not_to(change { PreorderReservationGroupsToTurn.count }) }
    ```

  * **Line # 228 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].
  ...
  * **Line # 237 - convention:** RSpec/IndexedLet: This `let` statement uses index in its name. Please give it a meaningful name.

    ```rb
        let(:turn3) { create(:reservation_turn, weekday: 2) }
    ```

  * **Line # 238 - convention:** RSpec/IndexedLet: This `let` statement uses index in its name. Please give it a meaningful name.

    ```rb
        let(:turn4) { create(:reservation_turn, weekday: 2) }
    ```

  * **Line # 253 - convention:** RSpec/ExpectChange: Prefer `change(PreorderReservationGroup, :count)`.

    ```rb
        it { expect { req }.to(change { PreorderReservationGroup.count }.by(1)) }
    ```

  * **Line # 254 - convention:** RSpec/ExpectChange: Prefer `change(PreorderReservationDate, :count)`.

    ```rb
        it { expect { req }.to(change { PreorderReservationDate.count }.by(5)) }
    ```

  * **Line # 255 - convention:** RSpec/ExpectChange: Prefer `change(PreorderReservationGroupsToTurn, :count)`.

    ```rb
        it { expect { req }.to(change { PreorderReservationGroupsToTurn.count }.by(1)) }
    ```

  * **Line # 267 - convention:** RSpec/MultipleExpectations: Example has too many expectations [3/1].

    ```rb
        it do
    ```

  * **Line # 293 - convention:** RSpec/RepeatedExampleGroupDescription: Repeated context block description on line(s) [312]

    ```rb
        context "when adding some turn to the new group, should receive 422" do ...
    ```

  * **Line # 307 - convention:** RSpec/ExpectChange: Prefer `change(PreorderReservationGroup, :count)`.

    ```rb
          it { expect { req }.not_to(change { PreorderReservationGroup.count }) }
    ```

  * **Line # 308 - convention:** RSpec/ExpectChange: Prefer `change(PreorderReservationDate, :count)`.

    ```rb
          it { expect { req }.not_to(change { PreorderReservationDate.count }) }
    ```

  * **Line # 309 - convention:** RSpec/ExpectChange: Prefer `change(PreorderReservationGroupsToTurn, :count)`.

    ```rb
          it { expect { req }.not_to(change { PreorderReservationGroupsToTurn.count }) }
    ```

  * **Line # 312 - convention:** RSpec/RepeatedExampleGroupDescription: Repeated context block description on line(s) [293]

    ```rb
        context "when adding some turn to the new group, should receive 422" do ...
    ```

  * **Line # 328 - convention:** RSpec/ExpectChange: Prefer `change(PreorderReservationGroup, :count)`.

    ```rb
          it { expect { req }.not_to(change { PreorderReservationGroup.count }) }
    ```

  * **Line # 329 - convention:** RSpec/ExpectChange: Prefer `change(PreorderReservationDate, :count)`.

    ```rb
          it { expect { req }.not_to(change { PreorderReservationDate.count }) }
    ```

  * **Line # 330 - convention:** RSpec/ExpectChange: Prefer `change(PreorderReservationGroupsToTurn, :count)`.

    ```rb
          it { expect { req }.not_to(change { PreorderReservationGroupsToTurn.count }) }
    ```

  * **Line # 361 - convention:** RSpec/PredicateMatcher: Prefer using `be_deferred` matcher over `deferred?`.

    ```rb
          it { expect(PreorderReservationGroup.last.deferred?).to be_truthy }
    ```

  * **Line # 368 - convention:** RSpec/RepeatedExample: Don't repeat examples within an example group.

    ```rb
        it { expect { req }.not_to(change { PreorderReservationGroup.count }) }
    ```

  * **Line # 368 - convention:** RSpec/ExpectChange: Prefer `change(PreorderReservationGroup, :count)`.

    ```rb
        it { expect { req }.not_to(change { PreorderReservationGroup.count }) }
    ```

  * **Line # 380 - convention:** RSpec/RepeatedExample: Don't repeat examples within an example group.

    ```rb
        it { expect { req }.not_to(change { PreorderReservationGroup.count }) }
    ```

  * **Line # 380 - convention:** RSpec/ExpectChange: Prefer `change(PreorderReservationGroup, :count)`.

    ```rb
        it { expect { req }.not_to(change { PreorderReservationGroup.count }) }
    ```

  * **Line # 381 - convention:** RSpec/ExpectChange: Prefer `change(PreorderReservationGroupsToTurn, :count)`.
  ...
  * **Line # 426 - convention:** RSpec/ExpectChange: Prefer `change(TableTypeToPreorderReservationGroup, :count)`.
...
### spec/requests/v1/admin/preorder_reservation_groups/preorder_reservation_groups.update_spec.rb - (19 offenses)
  ...
  * **Line # 91 - convention:** Style/SingleArgumentDig: Use `json[:item]` instead of `json.dig(:item)`.

    ```rb
          expect(json.dig(:item)).to include(:active_from)
    ```

  * **Line # 95 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].
  ...
  * **Line # 137 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].

    ```rb
        it do
    ```

  * **Line # 155 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].

    ```rb
          it do
    ```

  * **Line # 170 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].

    ```rb
        it do
    ```

  * **Line # 190 - convention:** RSpec/MultipleExpectations: Example has too many expectations [2/1].
  ...
  * **Line # 267 - convention:** RSpec/ExpectChange: Prefer `change(TableTypeToPreorderReservationGroup, :count)`.
...
### spec/requests/v1/nexi_controller/nexi_controller.receive_order_outcome_spec.rb - (132 offenses)
  * **Line # 5 - convention:** RSpec/Rails/InferredSpecType: Remove redundant spec type.
  ...
  * **Line # 132 - convention:** Layout/TrailingWhitespace: Trailing whitespace detected.

    ```rb
          it { 
    ```

  * **Line # 132 - convention:** Layout/MultilineBlockLayout: Block body expression is on the same line as the block start.

    ```rb
          it { req ...
    ```

  * **Line # 132 - convention:** Style/Semicolon: Do not use semicolons to terminate expressions.

    ```rb
          it { req; expect(ActionMailer::Base.deliveries.last.subject).to include("renotazione") }
    ```

  * **Line # 133 - convention:** Layout/IndentationWidth: Use 2 (not 8) spaces for indentation.

    ```rb
            req
    ```

  * **Line # 133 - convention:** Layout/IndentationConsistency: Inconsistent indentation detected.

    ```rb
     expect(ActionMailer::Base.deliveries.last.subject).to include("renotazione") }
    ```

  * **Line # 133 - convention:** RSpec/EmptyLineAfterExample: Add an empty line after `it`.

    ```rb
     expect(ActionMailer::Base.deliveries.last.subject).to include("renotazione") }
    ```

  * **Line # 133 - convention:** Layout/IndentationWidth: Use 2 (not -4) spaces for indentation.

    ```rb
      req ...
    ```

  * **Line # 133 - convention:** Style/Semicolon: Do not use semicolons to terminate expressions.

    ```rb
          it { req; expect(ActionMailer::Base.deliveries.last.subject).to include("registrata") }
    ```

  * **Line # 133 - convention:** Layout/BlockEndNewline: Expression at 133, 79 should be on its own line.

    ```rb
     expect(ActionMailer::Base.deliveries.last.subject).to include("renotazione") }
    ```

  * **Line # 134 - convention:** Layout/IndentationConsistency: Inconsistent indentation detected.
  ...
  * **Line # 134 - convention:** Layout/MultilineBlockLayout: Block body expression is on the same line as the block start.

    ```rb
          it { req ...
    ```

  * **Line # 134 - convention:** Style/Semicolon: Do not use semicolons to terminate expressions.

    ```rb
          it { req; expect(ActionMailer::Base.deliveries.last.html_part.decoded).to include("Il pagamento") }
    ```

  * **Line # 135 - convention:** Layout/BlockAlignment: `}` at 135, 0 is not aligned with `it { ` at 132, 6.

    ```rb
    }
    ```

  * **Line # 135 - convention:** Layout/IndentationConsistency: Inconsistent indentation detected.

    ```rb
     expect(ActionMailer::Base.deliveries.last.subject).to include("registrata") }
    ```

  * **Line # 135 - convention:** RSpec/EmptyLineAfterExample: Add an empty line after `it`.

    ```rb
     expect(ActionMailer::Base.deliveries.last.subject).to include("registrata") }
    ```

  * **Line # 135 - convention:** Style/Semicolon: Do not use semicolons to terminate expressions.

    ```rb
          it { req; expect(ActionMailer::Base.deliveries.last.html_part.decoded).to include("stato completato") }
    ```

  * **Line # 135 - convention:** Layout/BlockEndNewline: Expression at 135, 78 should be on its own line.

    ```rb
     expect(ActionMailer::Base.deliveries.last.subject).to include("registrata") }
    ```

  * **Line # 136 - convention:** Layout/MultilineBlockLayout: Block body expression is on the same line as the block start.

    ```rb
          it { req ...
    ```

  * **Line # 136 - convention:** Style/Semicolon: Do not use semicolons to terminate expressions.

    ```rb
          it { req; expect(ActionMailer::Base.deliveries.last.text_part.encoded).to include("Il pagamento") }
    ```

  * **Line # 137 - convention:** Layout/IndentationConsistency: Inconsistent indentation detected.

    ```rb
     expect(ActionMailer::Base.deliveries.last.html_part.decoded).to include("Il pagamento") }
    ```

  * **Line # 137 - convention:** RSpec/EmptyLineAfterExample: Add an empty line after `it`.

    ```rb
     expect(ActionMailer::Base.deliveries.last.html_part.decoded).to include("Il pagamento") }
    ```

  * **Line # 137 - convention:** Layout/TrailingWhitespace: Trailing whitespace detected.

    ```rb
          it { 
    ```

  * **Line # 137 - convention:** Style/Semicolon: Do not use semicolons to terminate expressions.

    ```rb
          it { req; expect(ActionMailer::Base.deliveries.last.text_part.encoded).to include("stato completato") }
    ```

  * **Line # 137 - convention:** Layout/BlockEndNewline: Expression at 137, 90 should be on its own line.

    ```rb
     expect(ActionMailer::Base.deliveries.last.html_part.decoded).to include("Il pagamento") }
    ```

  * **Line # 138 - convention:** Layout/IndentationWidth: Use 2 (not 8) spaces for indentation.

    ```rb
            req
    ```

  * **Line # 138 - convention:** Layout/IndentationWidth: Use 2 (not -4) spaces for indentation.

    ```rb
      req ...
    ```

  * **Line # 138 - convention:** Layout/MultilineBlockLayout: Block body expression is on the same line as the block start.

    ```rb
          it { req ...
    ```

  * **Line # 139 - convention:** Layout/IndentationConsistency: Inconsistent indentation detected.

    ```rb
     expect(ActionMailer::Base.deliveries.last.html_part.decoded).to include("stato completato") }
    ```

  * **Line # 139 - convention:** RSpec/EmptyLineAfterExample: Add an empty line after `it`.
  ...
  * **Line # 139 - convention:** Layout/BlockEndNewline: Expression at 139, 94 should be on its own line.

    ```rb
     expect(ActionMailer::Base.deliveries.last.html_part.decoded).to include("stato completato") }
    ```

  * **Line # 140 - convention:** Layout/BlockAlignment: `}` at 140, 0 is not aligned with `it { ` at 137, 6.

    ```rb
    }
    ```

  * **Line # 140 - convention:** Layout/MultilineBlockLayout: Block body expression is on the same line as the block start.

    ```rb
          it { req ...
    ```

  * **Line # 141 - convention:** Layout/IndentationConsistency: Inconsistent indentation detected.

    ```rb
     expect(ActionMailer::Base.deliveries.last.text_part.encoded).to include("Il pagamento") }
    ```

  * **Line # 141 - convention:** RSpec/EmptyLineAfterExample: Add an empty line after `it`.

    ```rb
     expect(ActionMailer::Base.deliveries.last.text_part.encoded).to include("Il pagamento") }
    ```

  * **Line # 141 - convention:** Layout/BlockEndNewline: Expression at 141, 90 should be on its own line.
  ...
  * **Line # 142 - convention:** Layout/MultilineBlockLayout: Block body expression is on the same line as the block start.

    ```rb
          it { req ...
    ```

  * **Line # 143 - convention:** Layout/IndentationWidth: Use 2 (not 8) spaces for indentation.

    ```rb
            req
    ```

  * **Line # 143 - convention:** Layout/IndentationConsistency: Inconsistent indentation detected.

    ```rb
     expect(ActionMailer::Base.deliveries.last.text_part.encoded).to include("stato completato") }
    ```

  * **Line # 143 - convention:** Layout/IndentationWidth: Use 2 (not -4) spaces for indentation.

    ```rb
      req ...
    ```

  * **Line # 143 - convention:** Layout/BlockEndNewline: Expression at 143, 94 should be on its own line.

    ```rb
     expect(ActionMailer::Base.deliveries.last.text_part.encoded).to include("stato completato") }
    ```

  * **Line # 144 - convention:** Layout/IndentationConsistency: Inconsistent indentation detected.
  ...
  * **Line # 145 - convention:** Layout/BlockAlignment: `}` at 145, 0 is not aligned with `it { ` at 142, 6.

    ```rb
    }
    ```

  * **Line # 147 - convention:** Layout/TrailingWhitespace: Trailing whitespace detected.

    ```rb
          it { 
    ```

  * **Line # 148 - convention:** Layout/IndentationWidth: Use 2 (not 8) spaces for indentation.

    ```rb
            req
    ```

  * **Line # 148 - convention:** Layout/IndentationWidth: Use 2 (not -4) spaces for indentation.

    ```rb
      req ...
    ```

  * **Line # 149 - convention:** Layout/IndentationConsistency: Inconsistent indentation detected.
  ...
  * **Line # 150 - convention:** Layout/BlockAlignment: `}` at 150, 0 is not aligned with `it { ` at 147, 6.

    ```rb
    }
    ```

  * **Line # 152 - convention:** Layout/TrailingWhitespace: Trailing whitespace detected.

    ```rb
          it { 
    ```

  * **Line # 153 - convention:** Layout/IndentationWidth: Use 2 (not 8) spaces for indentation.

    ```rb
            req
    ```

  * **Line # 153 - convention:** Layout/IndentationWidth: Use 2 (not -4) spaces for indentation.

    ```rb
      req ...
    ```

  * **Line # 154 - convention:** Layout/IndentationConsistency: Inconsistent indentation detected.
  ...
  * **Line # 155 - convention:** Layout/BlockAlignment: `}` at 155, 0 is not aligned with `it { ` at 152, 6.
  ...
  * **Line # 158 - convention:** Layout/IndentationWidth: Use 2 (not 8) spaces for indentation.

    ```rb
            req
    ```

  * **Line # 158 - convention:** Layout/IndentationWidth: Use 2 (not -4) spaces for indentation.

    ```rb
      req ...
    ```

  * **Line # 159 - convention:** Layout/IndentationConsistency: Inconsistent indentation detected.
  ...
  * **Line # 160 - convention:** Layout/BlockAlignment: `}` at 160, 0 is not aligned with `it { ` at 157, 6.
  ...
  * **Line # 216 - convention:** Style/Semicolon: Do not use semicolons to terminate expressions.

    ```rb
        it { req; expect(ActionMailer::Base.deliveries.last.html_part.decoded).to include("authorization") }
    ```

  * **Line # 217 - convention:** Style/Semicolon: Do not use semicolons to terminate expressions.

    ```rb
        it { req; expect(ActionMailer::Base.deliveries.last.text_part.decoded).to include("authorization") }
    ```

  * **Line # 222 - convention:** Layout/MultilineBlockLayout: Block body expression is on the same line as the block start.

    ```rb
        it { req ...
    ```

  * **Line # 223 - convention:** Layout/IndentationConsistency: Inconsistent indentation detected.

    ```rb
     expect(ActionMailer::Base.deliveries.last.html_part.decoded).to include("authorization") }
    ```

  * **Line # 223 - convention:** RSpec/EmptyLineAfterExample: Add an empty line after `it`.

    ```rb
     expect(ActionMailer::Base.deliveries.last.html_part.decoded).to include("authorization") }
    ```

  * **Line # 223 - convention:** Layout/BlockEndNewline: Expression at 223, 91 should be on its own line.

    ```rb
     expect(ActionMailer::Base.deliveries.last.html_part.decoded).to include("authorization") }
    ```

  * **Line # 224 - convention:** Layout/MultilineBlockLayout: Block body expression is on the same line as the block start.

    ```rb
        it { req ...
    ```

  * **Line # 225 - convention:** Layout/IndentationConsistency: Inconsistent indentation detected.

    ```rb
     expect(ActionMailer::Base.deliveries.last.text_part.decoded).to include("authorization") }
    ```

  * **Line # 225 - convention:** Layout/BlockEndNewline: Expression at 225, 91 should be on its own line.

    ```rb
     expect(ActionMailer::Base.deliveries.last.text_part.decoded).to include("authorization") }
    ```

  * **Line # 232 - convention:** Style/Semicolon: Do not use semicolons to terminate expressions.

    ```rb
        it { req; expect(ActionMailer::Base.deliveries.last.html_part.decoded).to include("payment") }
    ```

  * **Line # 233 - convention:** Style/Semicolon: Do not use semicolons to terminate expressions.

    ```rb
        it { req; expect(ActionMailer::Base.deliveries.last.text_part.decoded).to include("payment") }
    ```

  * **Line # 239 - convention:** RSpec/EmptyLineAfterExample: Add an empty line after `it`.

    ```rb
        it { expect { req }.to change { ActionMailer::Base.deliveries.count }.by(1) }
    ```

  * **Line # 239 - convention:** Layout/TrailingWhitespace: Trailing whitespace detected.

    ```rb
        it { 
    ```

  * **Line # 240 - convention:** Layout/IndentationWidth: Use 2 (not 6) spaces for indentation.

    ```rb
          req
    ```

  * **Line # 240 - convention:** Layout/IndentationWidth: Use 2 (not -2) spaces for indentation.

    ```rb
      req
    ```

  * **Line # 240 - convention:** Layout/MultilineBlockLayout: Block body expression is on the same line as the block start.

    ```rb
        it { req ...
    ```

  * **Line # 241 - convention:** Layout/IndentationConsistency: Inconsistent indentation detected.

    ```rb
     expect(ActionMailer::Base.deliveries.last.html_part.decoded).to include("payment") }
    ```

  * **Line # 241 - convention:** RSpec/EmptyLineAfterExample: Add an empty line after `it`.
  ...
  * **Line # 241 - convention:** Layout/BlockEndNewline: Expression at 241, 85 should be on its own line.

    ```rb
     expect(ActionMailer::Base.deliveries.last.html_part.decoded).to include("payment") }
    ```

  * **Line # 242 - convention:** Layout/BlockAlignment: `}` at 242, 0 is not aligned with `it { ` at 239, 4.

    ```rb
    }
    ```

  * **Line # 242 - convention:** Layout/MultilineBlockLayout: Block body expression is on the same line as the block start.

    ```rb
        it { req ...
    ```

  * **Line # 243 - convention:** Layout/IndentationConsistency: Inconsistent indentation detected.

    ```rb
     expect(ActionMailer::Base.deliveries.last.text_part.decoded).to include("payment") }
    ```

  * **Line # 243 - convention:** Layout/BlockEndNewline: Expression at 243, 85 should be on its own line.

    ```rb
     expect(ActionMailer::Base.deliveries.last.text_part.decoded).to include("payment") }
    ```

  * **Line # 244 - convention:** Layout/TrailingWhitespace: Trailing whitespace detected.

    ```rb
        it { 
    ```

  * **Line # 245 - convention:** Layout/IndentationWidth: Use 2 (not 6) spaces for indentation.

    ```rb
          req
    ```

  * **Line # 245 - convention:** Layout/IndentationWidth: Use 2 (not -2) spaces for indentation.

    ```rb
      req
    ```

  * **Line # 246 - convention:** Layout/IndentationConsistency: Inconsistent indentation detected.
  ...
  * **Line # 247 - convention:** Layout/BlockAlignment: `}` at 247, 0 is not aligned with `it { ` at 244, 4.

    ```rb
    }
    ```

  * **Line # 263 - convention:** Layout/TrailingWhitespace: Trailing whitespace detected.

    ```rb
        it { 
    ```

  * **Line # 264 - convention:** Layout/IndentationWidth: Use 2 (not 6) spaces for indentation.

    ```rb
          req
    ```

  * **Line # 264 - convention:** Layout/IndentationWidth: Use 2 (not -2) spaces for indentation.

    ```rb
      req
    ```

  * **Line # 265 - convention:** Layout/IndentationConsistency: Inconsistent indentation detected.
  ...
  * **Line # 266 - convention:** Layout/BlockAlignment: `}` at 266, 0 is not aligned with `it { ` at 263, 4.

    ```rb
    }
    ```

  * **Line # 268 - convention:** Layout/TrailingWhitespace: Trailing whitespace detected.

    ```rb
        it { 
    ```

  * **Line # 269 - convention:** Layout/IndentationWidth: Use 2 (not 6) spaces for indentation.

    ```rb
          req
    ```

  * **Line # 269 - convention:** Layout/IndentationWidth: Use 2 (not -2) spaces for indentation.

    ```rb
      req
    ```

  * **Line # 270 - convention:** Layout/IndentationConsistency: Inconsistent indentation detected.
  ...
  * **Line # 271 - convention:** Layout/BlockAlignment: `}` at 271, 0 is not aligned with `it { ` at 268, 4.
...
### spec/requests/v2/reservations_controller/valid_times_spec.rb - (28 offenses)
  ...
  * **Line # 434 - convention:** Layout/LineLength: Line is too long. [181/120]
  ...
  * **Line # 483 - convention:** Rails/TimeZone: Do not use `Time.now` without zone. Use one of `Time.zone.now`, `Time.current`, `Time.now.in_time_zone`, `Time.now.utc`, `Time.now.getlocal`, `Time.now.xmlschema`, `Time.now.iso8601`, `Time.now.jisx0301`, `Time.now.rfc3339`, `Time.now.httpdate`, `Time.now.to_i`, `Time.now.to_f` instead.
  ...
  * **Line # 575 - convention:** Rails/TimeZone: Do not use `Time.now` without zone. Use one of `Time.zone.now`, `Time.current`, `Time.now.in_time_zone`, `Time.now.utc`, `Time.now.getlocal`, `Time.now.xmlschema`, `Time.now.iso8601`, `Time.now.jisx0301`, `Time.now.rfc3339`, `Time.now.httpdate`, `Time.now.to_i`, `Time.now.to_f` instead.

    ```rb
          ReservationTurn.create!(name: "Night", weekday: Time.now.wday, starts_at: "19:00", ends_at: "21:00", step: 30)
    ```

  * **Line # 756 - convention:** RSpec/RepeatedExample: Don't repeat examples within an example group.

    ```rb
        it { expect(json.dig(:turns, 0, :messages)).to all(include(message: String)) }
    ```

  * **Line # 757 - convention:** RSpec/RepeatedExample: Don't repeat examples within an example group.
